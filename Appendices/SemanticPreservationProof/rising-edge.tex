...
\begin{definition}[Rising edge hypotheses]
  ...
  $\sigma_e,\sigma,\sigma_i,\sigma_\uparrow,\sigma'\in\Sigma$,
  assume that:
  \begin{itemize}
...
\begin{niproof}
  ...
  \fbox{$s.M(p)-\sum\limits_{t\in{}Fired(s)}pre(p,t)+\sum\limits_{t\in{}Fired(s)}post(t,p)=\sigma(id_p)(\texttt{sm})-\sigma(id_p)(\texttt{sots})
...
\begin{proof}
  ...
  \item \textbf{CASE} $conds(t)=\emptyset$:
    ...
    By construction,
    ${<}\mathtt{conditions\_number\Rightarrow{}1}{>}\in{}g_t$ and
    ${<}\mathtt{input\_conditions(0)\Rightarrow{}true}{>}\in{}i_t$.

    By property of the stabilize relation,
    ${<}\mathtt{conditions\_number\Rightarrow{}1}{>}\in{}g_t$ and
    ${<}$\texttt{input\_conditions(0)}$\mathtt{\Rightarrow{}true}{>}\in{}i_t$,
    we can deduce $\Delta(id_t)(\texttt{cn})=1$ and
  ...
  \item \textbf{CASE} $conds(t)\neq\emptyset$:\\
    By construction,
    ${<}\mathtt{conditions\_number\Rightarrow{}\vert{}conds(t)\vert}{>}\in{}g_t$,
    ...
    \item \textbf{BASE CASE}:
      $\prod\limits_{i=0}^{\vert{}conds(t)\vert-1}\sigma'(id_t)(\texttt{ic})[i]=0$
      and $\vert{}conds(t)\vert-1<0$. Thus, we can deduce that
      $\vert{}conds(t)\vert=0$ which \qedbox{contradicts
        $conds(t)\neq\emptyset$.}
    \item \textbf{INDUCTION CASE}:
      \begin{ih}
        $\forall{}conds'\subseteq{}\mathcal{C},~\prod\limits_{i=1}^{\vert{}conds(t)\vert-1}\sigma'(id_t)(\texttt{ic})[i]=
      ...
      \fbox{$\sigma'(id_t)(\texttt{ic})[0]~.~\prod\limits_{i=1}^{\vert{}conds(t)\vert-1}\sigma'(id_t)(\texttt{ic})[i]=
        \prod\limits_{c\in{}conds(t)}
        \begin{cases}
          E_c(\tau,c) & if~\mathbb{C}(t,c)=1 \\
          \mathtt{not}(E_c(\tau,c)) & if~\mathbb{C}(t,c)=-1 \\
      ...
      \item $\mathbb{C}(t,c)=1$ implies
        ${<}\mathtt{input\_conditions(i)\Rightarrow{}id_c}{>}\in{}i_t$
      \item $\mathbb{C}(t,c)=-1$ implies
        ${<}\mathtt{input\_conditions(i)\Rightarrow{}not~id_c}{>}\in{}i_t$
      \end{itemize}
      ...
      \item \textbf{CASE} $\mathbb{C}(t,c)=1$:
        ...
        Then, we have
        ${<}\mathtt{input\_conditions(0)\Rightarrow{}id_c}{>}\in{}i_t$
        and by property of the stabilize relation, we can deduce
        $\sigma(id_t)(\texttt{ic})[0]=\sigma'(id_c)$.

        Rewriting the goal with $\sigma(id_t)(\texttt{ic})[0]=\sigma'(id_c)$:

        \fbox{$\sigma'(id_c)~.~\prod\limits_{i=1}^{\vert{}conds(t)\vert-1}\sigma'(id_t)(\texttt{ic})[i]=
          \prod\limits_{c\in{}conds(t)}
          \begin{cases}
            E_c(\tau,c) & if~\mathbb{C}(t,c)=1 \\
            \mathtt{not}(E_c(\tau,c)) & if~\mathbb{C}(t,c)=-1 \\
        ...
        \fbox{$E_c(\tau,c)~.~\prod\limits_{i=1}^{\vert{}conds(t)\vert-1}\sigma'(id_t)(\texttt{ic})[i]=
          \prod\limits_{c\in{}conds(t)}
          \begin{cases}
            E_c(\tau,c) & if~\mathbb{C}(t,c)=1 \\
            \mathtt{not}(E_c(\tau,c)) & if~\mathbb{C}(t,c)=-1 \\
        ...
        \begin{frameb}
          \begin{tabular}{c}
            $E_c(\tau,c)~.~\prod\limits_{i=1}^{\vert{}conds(t)\vert-1}\sigma'(id_t)(\texttt{ic})[i]$ \\
            $=$ \\
      ...
      \item \textbf{CASE} $\mathbb{C}(t,c)=-1$:
        ...
        Then, we have
        ${<}$\texttt{input\_conditions(0)}$\mathtt{\Rightarrow{}not~id_c}{>}\in{}i_t$
        and by property of the stabilize relation, we can deduce
        $\sigma(id_t)(\texttt{ic})[0]=\mathtt{not}~\sigma'(id_c)$.

        Rewriting the goal with
        $\sigma(id_t)(\texttt{ic})[0]=\mathtt{not}~\sigma'(id_c)$:

        \fbox{$\mathtt{not}~\sigma'(id_c)~.~\prod\limits_{i=1}^{\vert{}conds(t)\vert-1}\sigma'(id_t)(\texttt{ic})[i]=
          \prod\limits_{c\in{}conds(t)}
          \begin{cases}
            E_c(\tau,c) & if~\mathbb{C}(t,c)=1 \\
            \mathtt{not}(E_c(\tau,c)) & if~\mathbb{C}(t,c)=-1 \\
        ...
        \fbox{$\mathtt{not}~E_c(\tau,c)~.~\prod\limits_{i=1}^{\vert{}conds(t)\vert-1}\sigma'(id_t)(\texttt{ic})[i]=
          \prod\limits_{c\in{}conds(t)}
          \begin{cases}
            E_c(\tau,c) & if~\mathbb{C}(t,c)=1 \\
            \mathtt{not}(E_c(\tau,c)) & if~\mathbb{C}(t,c)=-1 \\
        ...
        \begin{frameb}
          \begin{tabular}{c}
            $\mathtt{not}~E_c(\tau,c)~.~\prod\limits_{i=1}^{\vert{}conds(t)\vert-1}\sigma'(id_t)(\texttt{ic})[i]$ \\
            $=$ \\
...
\begin{niproof}
  ...
  \item
    \fbox{$upper(I_s(t))\neq\infty\land{}s'.I(t)\le{}upper(I_s(t))\Rightarrow{}s'.I(t)=\sigma'(id_t)(\texttt{s\_time\_counter})$}
  ...
  \item \textbf{CASE} $input(t)\neq{}\emptyset$:
    ...
    \item \textbf{CASE} $t\notin{}Fired(s)$: Then, there are two cases
      ...
      \item \textbf{CASE} there exists such a place $p$ as described
        ...
        \begin{equation*}
          \fbox{$\big((s.M(p)-\sum\limits_{t_i\in{}Fired(s)}pre(p,t_i)<\omega)~.~(\sum\limits_{t_i\in{}Fired(s)}pre(p,t_i)>0)\big)+\sigma(id_t)(\texttt{fired})=\mathtt{true}$}
        \end{equation*}
        ...
        assumption:
        \begin{equation*}
          \qedbox{$\big((s.M(p)-\sum\limits_{t_i\in{}Fired(s)}pre(p,t_i)<\omega)~.~(\sum\limits_{t_i\in{}Fired(s)}pre(p,t_i)>0)\big)+\sigma(id_t)(\texttt{fired})=\mathtt{true}$}
        \end{equation*}
