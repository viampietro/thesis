...
\begin{lemma}[First rising edge equal time counters]
  ...
  \fstrehyps{} then\\
  $\forall{}t\in{}T_i,id_t\in{}Comps(\Delta)~s.t.~\gamma(t)=id_t$,\\
  $upper(I_s(t))=\infty\land{}s_0.I(t)\le{}lower(I_s(t))\Rightarrow{}s_0.I(t)=\sigma(id_t)("s\_time\_counter")\land{}$\\
  $upper(I_s(t))=\infty\land{}s_0.I(t)>{}lower(I_s(t))\Rightarrow{}\sigma(id_t)("s\_time\_counter")=lower(I_s(t))\land{}$\\
  $upper(I_s(t))\neq\infty\land{}s_0.I(t)>{}upper(I_s(t))\Rightarrow{}\sigma(id_t)("s\_time\_counter")=upper(I_s(t))\land{}$\\
  $upper(I_s(t))\neq\infty\land{}s_0.I(t)\le{}upper(I_s(t))\Rightarrow{}s_0.I(t)=\sigma(id_t)("s\_time\_counter")$.
\end{lemma}
...
\begin{niproof}
  ...
  \begin{enumerate}
  \item \framebox{$upper(I_s(t))=\infty\land{}s_0.I(t)\le{}lower(I_s(t))\Rightarrow{}s_0.I(t)=\sigma(id_t)("s\_time\_counter")$}
  \item \framebox{$upper(I_s(t))=\infty\land{}s_0.I(t)>{}lower(I_s(t))\Rightarrow{}\sigma(id_t)("s\_time\_counter")=lower(I_s(t))$}
  \item \framebox{$upper(I_s(t))\neq\infty\land{}s_0.I(t)>{}upper(I_s(t))\Rightarrow{}\sigma(id_t)("s\_time\_counter")=upper(I_s(t))$}
  \item \framebox{$upper(I_s(t))\neq\infty\land{}s_0.I(t)\le{}upper(I_s(t))\Rightarrow{}s_0.I(t)=\sigma(id_t)("s\_time\_counter")$}
  \end{enumerate}
  ...
  \item Assuming that $upper(I_s(t))=\infty$ and
    ...
    \framebox{${}s_0.I(t)=\sigma(id_t)("stc")$.}
    
    By property of the $\mathtt{Inject_\uparrow}$ relation, the
    \hvhdl{} rising edge and stabilize relations, and \InCsCompT{}, we
    can deduce $\sigma(id_t)("stc")=\sigma_0(id_t)("stc")$.
    
    Rewriting $\sigma(id_t)("stc")$ as $\sigma_0(id_t)("stc")$,
    \fbox{${}s_0.I(t)=\sigma_0(id_t)("stc")$.}

    Appealing to Lemma~\ref{lem:init-states-eq-tc},
    \qedbox{${}s_0.I(t)=\sigma_0(id_t)("stc")$.}
  
  \item Assuming that $upper(I_s(t))=\infty$ and
    ${}s_0.I(t)>{}lower(I_s(t))$, let us show\\
    \framebox{$\sigma(id_t)("stc")=lower(I_s(t))$.}
  ...
  \item Assuming that $upper(I_s(t))\neq\infty$ and
    ...
    \framebox{$\sigma(id_t)("stc")=upper(I_s(t))$}.
...
\begin{niproof}
  ...
  \begin{equation}
    \sigma(id_t)("srtc")=\sum\limits_{i=0}^{\Delta(id_t)("input\_arcs\_number")-1}\sigma(id_t)("reinit\_time")[i]\label{eq:srtc-at-init-states}
  \end{equation}

  Rewriting the goal with Equation~\eqref{eq:srtc-at-init-states}:
  \fbox{$s_0.reset_t(t)=\sum\limits_{i=0}^{\Delta(id_t)("ian")-1}\sigma(id_t)("rt")[i]$.}
  ...
  \item \textbf{CASE} $input(t)=\emptyset$:
    ...
    By construction,
    $<\mathtt{reinit\_time(0)\Rightarrow{}false}>\in{}ipm_t$, and by
    property of the \hvhdl{} stabilize relation,
    $\sigma(id_t)("rt")[0]=\mathtt{false}$.
  ...
  \item \textbf{CASE} $input(t)\neq{}\emptyset$:
    ...
    s.t. $\gamma(p)=id_p$, a $gm_p,$ an $ipm_p,$ an $opm_p$
    ...
    ${<}$\texttt{reinit\_transition\_time(j)$\Rightarrow{}id_{ji}$}${>}\in{}opm_p$
    and
    ${<}$\texttt{reinit\_time(i)$\Rightarrow{}id_{ji}$}${>}\in{}ipm_t$,
    we can deduce
    ...
    \begin{equation*}
      \fbox{$\begin{split}
          \mathtt{false}=& ((\sigma_0(id_p)("oat")[j]=\mathtt{basic}+\sigma_0(id_p)("oat")[j]=\mathtt{test}) \\
          & .(\sigma_0(id_p)("sm")-\sigma_0(id_p)("sots")<\sigma_0(id_p)("oaw")[j])\\
          & .(\sigma_0(id_p)("sots")>0))\\
          & +(\sigma_0(id_p)("otf")[j]) \\
        \end{split}$}
    ...
    ${<}$\texttt{fired}$\Rightarrow{}\mathtt{id_{fj}}{>}\in{}opm_t$
    and
    ${<}$\texttt{output\_transitions\_fired(j)}$\Rightarrow{}\mathtt{id_{fj}}{>}\in{}ipm_p$.
    ...
    ${<}$\texttt{output\_transitions\_fired(j)}$\Rightarrow{}\mathtt{id_{fj}}{>}\in{}ipm_p$,
    we can deduce
    $\sigma_0(id_p)("otf")[j]=\sigma_0(id_{fj})=\sigma_0(id_t)("fired")$.
    
    Appealing to Lemma~\ref{lem:init-states-fired-false}, we can
    deduce $\sigma_0(id_t)("fired")=\mathtt{false}$ and consequently
    $\sigma_0(id_p)("otf")[j]=\mathtt{false}$.

    Rewriting $\sigma_0(id_p)("otf")[j]$ as $\mathtt{false}$ and
    ...
    Rewriting $\sigma_0(id_p)("sots")$ as $0$ and simplifying the
...
\begin{niproof}
  ...
  \noindent{}By property of the $\mathtt{Inject_{\uparrow}}$, \hvhdl{}
...
\begin{niproof}
  ...
  \item \textbf{CASE} $trs(f)\neq\emptyset$:
    ...
    $gm_{t_i}$, $ipm_{t_i}$ and $opm_{t_i}$ s.t.
    $\mathtt{comp}($$id_{t_i},$ $"transition",$
    $gm_{t_i},$ $ipm_{t_i},$ $opm_{t_i})\in{}d.cs$.
    ...
    $\sigma_0(id_{t_i})("fired")$,
    \fbox{$\sigma_0(id_{t_0})("fired")+\dots+\sigma_0(id_{t_n})("fired")=\mathtt{false}$.}

    Appealing to Lemma~\ref{lem:init-states-fired-false}, we can
    deduce $\sigma_0(id_{t_i})("fired")=\mathtt{false}$.

    Rewriting all $\sigma_0(id_{t_i})("fired")$ as $\mathtt{false}$
