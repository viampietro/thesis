...
\begin{niproof}
  ...
  \texttt{marking} process defined in the place design architecture,
  we can deduce
  ...
  \qedbox{$s.M(p)=\sigma(id_p)(\texttt{s\_marking})$.}
   ...
   \item \textbf{INDUCTION CASE}:
     ...
     \begin{equation}
       \label{eq:31}g(0)=\begin{cases}
         \sigma'(id_p)(\texttt{oaw})[0]~\mathtt{if}~(\sigma'(id_p)(\texttt{otf})[0] \\
         \hspace{19.5ex}.~\sigma'(id_p)(\texttt{oat})[0]=\mathtt{basic}) \\
         0~otherwise \\
       \end{cases}
     \end{equation}

     \noindent{}Let us perform case analysis on the value of
     $\sigma'(id_p)(\texttt{otf})[0]~.~\sigma'(id_p)(\texttt{oat})[0]=\mathtt{basic}$; there are two cases:
     \begin{enumerate}
     \item $(\sigma'(id_p)(\texttt{otf})[0]~.~\sigma'(id_p)(\texttt{oat})[0]=\mathtt{basic})=\mathtt{false}$:\\
     ...
     \item $(\sigma'(id_p)(\texttt{otf})[0]~.~\sigma'(id_p)(\texttt{oat})[0]=\mathtt{basic})=\mathtt{true}$:\\
       In that case, $g(0)=\sigma'(id_p)(\texttt{oaw})[0]$,
       $\sigma'(id_p)(\texttt{otf})[0]=\mathtt{true}$ and
       $\sigma'(id_p)(\texttt{oat})[0]=\mathtt{basic}$.

       \noindent{}By construction, there exist a $t\in{}output(t)$,
       $id_t\in{}Comps(\Delta)$ s.t. $\gamma(t)=id_t$, and there exist
       $g_t$, $i_t$, $o_t$ s.t. $\mathtt{comp}(id_t,$
       $\texttt{transition},$ $g_t,$ $i_t,$ $o_t)\in{}d.cs$, and there
       ...
       \item ${<}\mathtt{output\_arcs\_types(0)\Rightarrow{}}a{>}\in{}i_p$
       \item ${<}\mathtt{output\_arcs\_weights(0)\Rightarrow{}}\omega{>}\in{}i_p$
       \item ${<}\mathtt{fired\Rightarrow}id_{ft}{>}\in{}o_t$
       \item
         ${<}\mathtt{output\_transitions\_fired(0)\Rightarrow{}id_{ft}}{>}\in{}i_p$
       \end{itemize}
       
       \noindent{}By property of the stabilize relation,
       $\sigma'(id_p)(\texttt{oat})[0]=\mathtt{basic}$ and
       ${<}$\texttt{output\_arcs\_types(0)}$\mathtt{\Rightarrow{}a}{>}\in{}i_p$,
       we can deduce $pre(p,t)=(\omega,\mathtt{basic})$.

       \noindent{}By property of the stabilize relation,
       ${<}\mathtt{fired\Rightarrow{}id_{ft}}{>}\in{}o_t$,\\
       ${<}\mathtt{output\_transitions\_fired(0)\Rightarrow{}id_{ft}}{>}\in{}i_p$
       and $\sigma'(id_p)(\texttt{otf})[0]=\mathtt{true}$, we can deduce
       $\sigma'(id_t)(\texttt{fired})=\mathtt{true}$.

       Appealing to Lemma~\ref{lem:fe-equal-fired}, and thanks to
       $\sigma'(id_t)(\texttt{fired})=\mathtt{true}$, we can deduce
       $t\in{}Fired(s')$.
       ...
       \noindent{}We know that $g(0)=\sigma'(id_p)(\texttt{oaw})[0]$, and by
       property of the stabilize relation and
       ${<}$\texttt{output\_arcs\_weights(0)}$\Rightarrow\omega{>}\in{}i_p$,
       we can deduce $\sigma'(id_p)(\texttt{oaw})[0]=\omega$.

       Rewriting the goal with $\sigma'(id_p)(\texttt{oaw})[0]=\omega$:\\
...
\begin{niproof}
  ...
  \item \textbf{CASE} $input(p)\neq\emptyset$:
    ...
    \noindent{}By construction,
    ${<}\mathtt{input\_arcs\_number\Rightarrow{}}\vert{}input(p)\vert{>}\in{}g_p$,
    and by property of the elaboration relation, we can deduce
    ...
    \item \textbf{INDUCTION CASE}: In that case,
      ...
      $g(0)=\begin{cases}
        \sigma'(id_p)(\texttt{iaw})[0]~\mathtt{if}~\sigma'(id_p)(\texttt{itf})[0]\\
        0~otherwise \\
      \end{cases}$

      \noindent{}Let us perform case analysis on the value of
      $\sigma'(id_p)(\texttt{itf})[0]$; there are two cases:
      \begin{enumerate}
      \item $\sigma'(id_p)(\texttt{itf})[0]=\mathtt{false}$:\\
        In that case, $g(0)=0$, and then we can apply the induction
        hypothesis with $F=Fired(s')$ to solve the goal:
        \qedbox{$\sum\limits_{t\in{}Fired(s')}f(t)=\sum\limits_{i=1}^{\vert{}input(p)\vert-1}g(i)$.}

      \item $\sigma'(id_p)(\texttt{itf})[0]=\mathtt{true}$:\\
        In that case, $g(0)=\sigma'(id_p)(\texttt{iaw})[0]$ and
        $\sigma'(id_p)(\texttt{itf})[0]=\mathtt{true}$ .

        \noindent{}By construction, there exist a $t\in{}input(t)$,
        an $id_t\in{}Comps(\Delta)$ s.t. $\gamma(t)=id_t$, $g_t$,
        $i_t$, $o_t$ s.t. $\mathtt{comp}(id_t,$ $\texttt{transition},$
        $g_t,$ $i_t,$ $opm)\in{}d.cs$, an
        $\omega\in\mathbb{N}^{*}$ and an $id_{ft}\in{}Sigs(\Delta)$
        such that:
        \begin{itemize}
        \item $post(t,p)=\omega$
        \item
          ${<}\mathtt{input\_arcs\_weights(0)\Rightarrow{}}\omega{>}\in{}i_p$
        \item ${<}\mathtt{fired\Rightarrow}id_{ft}{>}\in{}o_t$
        \item
        ...
        ${<}$\texttt{fired}$\Rightarrow{}\mathtt{id_{ft}}{>}\in{}o_t$,
        ${<}$\texttt{input\_transitions\_fired(0)}$\mathtt{\Rightarrow{}id_{ft}}{>}\in{}i_p$
        and $\sigma'(id_p)(\texttt{itf})[0]=\mathtt{true}$, we can deduce
        $\sigma'(id_t)(\texttt{fired})=\mathtt{true}$.
        ...
        We know that $g(0)=\sigma'(id_p)(\texttt{iaw})[0]$, and by property
        of the stabilize relation and
        ${<}$\texttt{input\_arcs\_weights(0)}$\Rightarrow\omega{>}\in{}i_p$,
        we can deduce $\sigma'(id_p)(\texttt{iaw})[0]=\omega$.

        Rewriting the goal with $\sigma'(id_p)(\texttt{iaw})[0]=\omega$:\\
...
\begin{niproof}
  ...
  \item\label{it:fe-eq-tc-fst}
    \fbox{$upper(I_s(t))=\infty\land{}s'.I(t)\le{}lower(I_s(t))\Rightarrow{}s'.I(t)=\sigma'(id_t)(\texttt{s\_time\_counter})$}\\
    ...
    \item \textbf{CASE} $t\in{}Sens(s.M)$:
      ...
      ${<}\mathtt{transition\_type\Rightarrow{}TEMP\_A\_INF}{>}\in{}g_t$. By
  ...
  \item
    \fbox{$upper(I_s(t))=\infty\land{}s'.I(t)>{}lower(I_s(t))\Rightarrow$
      $\sigma'(id_t)(\texttt{s\_time\_counter})=lower(I_s(t)$.}

    Assuming that $upper(I_s(t))=\infty$ and
    $s'.I(t)>{}lower(I_s(t))$, let us show\\
    \fbox{$\sigma'(id_t)(\texttt{s\_time\_counter})=lower(I_s(t))$.}
    ...
    \item \textbf{CASE} $t\in{}Sens(s.M)$:
      ...
      \item \textbf{CASE} $s.reset_t(t)=\mathtt{false}$:
        ...
        \begin{enumerate}
        \item \textbf{CASE} $s.I(t)>lower(I_s(t))$: \fbox{$\sigma'(id_t)(\texttt{stc})=lower(I_s(t))$.}\\
          By definition of
          $\gamma,E_c,\tau\vdash{}s\stackrel{\uparrow}{\approx}\sigma$,
          we can deduce $\sigma(id_t)(\texttt{stc})=lower(I_s(t))$.
          ...
          $\sigma(id_t)(\texttt{stc})=lower(I_s(t))$: \qedbox{tautology.}
        ...
        \item \textbf{CASE} $s.I(t)=lower(I_s(t))$: \fbox{$\sigma'(id_t)(\texttt{stc})=lower(I_s(t))$.}\\
          ...
          Rewriting the goal with
          $\sigma'(id_t)(\texttt{stc})=\sigma(id_t)(\texttt{stc})$,
          $s.I(t)=\sigma(id_t)(\texttt{stc})$ and $s.I(t)=lower(I_s(t))$:
          \qedbox{tautology.}
        \end{enumerate}
      \end{enumerate}
    \end{enumerate}
  \item
    \fbox{$upper(I_s(t))\neq\infty\land{}s'.I(t)>{}upper(I_s(t))\Rightarrow$
      $\sigma'(id_t)(\texttt{s\_time\_counter})=upper(I_s(t))$.}

    Assuming that $upper(I_s(t))\neq\infty$ and
    $s'.I(t)>{}upper(I_s(t))$, let us show \\
    \fbox{$\sigma'(id_t)(\texttt{s\_time\_counter})=upper(I_s(t))$.}
    ...
    deduce $\Delta(id_t)(\texttt{mtc})=b=upper(I_s(t))$ and
    $\Delta(id_t)(\texttt{tt})\neq\mathtt{NOT\_TEMP}$.
    ...
    Let us perform case analysis on $t\in{}Sens(s.M)$:
      ...
      \item \textbf{CASE} $t\in{}Sens(s.M)$:\\
        ...
        \item \textbf{CASE} $s.reset_t(t)=\mathtt{false}$:\\
          ...
          \begin{enumerate}
          \item \textbf{CASE} $s.I(t)>upper(I_s(t))$: \fbox{$\sigma'(id_t)(\texttt{stc})=upper(I_s(t))$.}\\
            By definition of \dwSitpn (Rule~\ref{it:locked-counters}),
            we can deduce $s'.I(t)=s.I(t)$.
            
            By definition of
            $\gamma,E_c,\tau\vdash{}s\stackrel{\uparrow}{\approx}\sigma$,
            we can deduce $\sigma(id_t)(\texttt{stc})=upper(I_s(t))$.
            ...
            Rewriting the goal with
            $\sigma'(id_t)(\texttt{stc})=\sigma(id_t)(\texttt{stc})$ and
            $\sigma(id_t)(\texttt{stc})=upper(I_s(t))$: \qedbox{tautology.}
            
          \item \textbf{CASE} $s.I(t)\le{}upper(I_s(t))$: \fbox{$\sigma'(id_t)(\texttt{stc})=upper(I_s(t))$.}\\
            ...
            \item \textbf{CASE} $s.I(t)=upper(I_s(t))$:\\
              ...
              Appealing to $\Delta(id_t)(\texttt{mtc})=b=upper(I_s(t))$,
              $s.I(t)=\sigma(id_t)(\texttt{stc})$ and $s.I(t)=upper(I_s(t))$,
              we can deduce
    ...
    \item
      ...
      $\Delta(id_t)(\texttt{mtc})=b=upper(I_s(t))$ and
      $\Delta(id_t)(\texttt{tt})\neq\mathtt{NOT\_TEMP}$.
        ...
        \item \textbf{CASE} $s.reset_t(t)=\mathtt{false}$:\\
          ...
          \item \textbf{CASE} $s.I(t)\le{}upper(I_s(t))$:\\
            ...
            \item \textbf{CASE} $s.I(t)<upper(I_s(t))$:\\
              ...
              $\Delta(id_t)(\texttt{mtc})=b=upper(I_s(t))$, we can deduce
              $\sigma(id_t)(\texttt{stc})<\Delta(id_t)(\texttt{mtc})$.
...
\begin{niproof}
  ...
  \begin{equation}
    \label{eq:checktc}
    \begin{split}
      \mathtt{checktc}(\Delta(id_t),\sigma(id_t))=&\bigg(\mathtt{not}~\sigma(id_t)(\texttt{srtc})~.~\\
      & \quad\begin{split}
      ...
      \item \textbf{CASE} $s.reset_t(t)=\mathtt{false}$: \fbox{$\mathtt{checktc}(\Delta(id_t),\sigma(id_t))=\mathtt{true}$}\\
        ...
        \item \textbf{CASE} $I_s(t)=[a,\infty]$ where
          ...
          \item \textbf{CASE} $s.I(t)>lower(I_s(t))$:
            ...
            By definition of \upSim, we have
            $\sigma(id_t)(\texttt{stc})=lower(I_s(t))=a$.
    ...
    \item \textbf{CASE}
      ...
      \item \textbf{CASE}
        ...
        \item \textbf{CASE} $s.I(t)>upper(I_s(t))$:
          ...
          By definition of \upSim, we have
          $\sigma(id_t)(\texttt{stc})=upper(I_s(t))=b$.

          Then, from $\sigma(id_t)(\texttt{stc})\le{}\sigma(id_t)(\texttt{B})-1$,
          $\sigma(id_t)(\texttt{stc})=upper(I_s(t))=b$ and
      ...
      \item
        ...
        \item \textbf{CASE} $s.I(t)>upper(I_s(t))$:
          ...
          By definition of \upSim, we have
          $\sigma(id_t)(\texttt{stc})=upper(I_s(t))=a$.
