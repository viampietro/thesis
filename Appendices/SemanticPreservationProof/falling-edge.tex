    ...
    \item \fbox{\parbox{\lwidth}{$\forall{}t'\in{}T,id_{t'}\in{}Comps(\Delta)$ s.t. $\gamma(t')=id_{t'}$,\\
      ...
      \item Assuming that $t'\in{}fired\cup\{t_0\}$, let us show
        ...
        \item \textbf{CASE} $t'=t_0$:
          ...
          $\mathtt{comp}(id_{t'},"transition",$ $gm_{t'},$ $ipm_{t'}$,
          $opm_{t'})\in{}d.cs$, and through the examination of the
          \texttt{fired_evaluation} process defined in the transition
          design architecture:
          \begin{equation}
            \label{eq:frd-eq-frd}
          ...
          \begin{enumerate}
          \item \fbox{$\sigma(id_{t'})("sfa")=\mathtt{true}$.} 
          \item \fbox{$\sigma(id_{t'})("spc")=\mathtt{true}$.} 
          \end{enumerate}
          ...
          \item \fbox{$\sigma(id_{t'})("sfa")=\mathtt{true}$.}
            ...
            Appealing to Lemma~\ref{lem:fe-equal-firable}, we can
            deduce \qedbox{$\sigma(id_{t'})("sfa")=\mathtt{true}$.}
          \item \fbox{$\sigma(id_{t'})("spc")=\mathtt{true}$.}

            Appealing to Lemma~\ref{lem:stab-compute-pcomb}, we can
            deduce \qedbox{$\sigma(id_{t'})("spc")=\mathtt{true}$.}
          \end{enumerate}
...
\begin{niproof}
  ...
  \item\label{item:stab-comp-spc-fst-case}
    ...
    \item \textbf{CASE} $input(t)=\emptyset$:
      ...
      ${<}$\texttt{priority\_authorizations(0)}$\Rightarrow{}\mathtt{true}{>}\in{}ipm_t$.
    ...
    \item \textbf{CASE} $input(t)\neq{}\emptyset$:
      ...
      \item \textbf{CASE} $pre(p,t)=(\omega,\mathtt{basic})$:
        ...
        \item \textbf{CASE} The priority relation is a strict total
          ...
          process defined in the place design behavior, we can deduce:
          \begin{equation}
            \label{eq:frd-eq-pauthsj}
            \sigma'(id_p)("pauths")[j]=(\sigma'(id_p)("sm")\ge{}\mathtt{vsots}+\sigma'(id_p)("oaw")[j])
          \end{equation}
          ...
          \fbox{$\sigma'(id_p)("sm")\ge{}\mathtt{vsots}+\sigma'(id_p)("oaw")[j]$}
          ...
          \item
            \fbox{$\sum\limits_{t_i\in{}Pr(t,fired)}pre(p,t_i)=\mathtt{vsots}$}\\
            ...
            By property of the well-definition of $sitpn$, the
            priority relation is a strict total order over the
            transitions of set $output_c(p)$. This ordering is
            reflected in the ordering of the indexes of the output
            port \texttt{priority\_authorizations} for each place
            component instance. Thus, in the
            \texttt{priority\_authorizations} output port of a place
            component instance, the element of index 0 is connected to
            the transition of $output_c(t)$ with the highest firing
            priority. We know that component $id_t$ is connected to
            \texttt{priority\_authorizations(0)} in the output port
            map of component $id_p$. By construction, transition $t$
            is the transition of $output_c(p)$ with the highest firing
            priority, i.e,
            ...
            $output_c(p)$, there exist a $t_0\in{}output_c(p)$, an
            $id_{t_0}\in{}Comps(\Delta)$, $gm_{t_0}$, $ipm_{t_0}$
            $opm_{t_0}$, and an $id_{ft_0}\in{}Sigs(\Delta)$ such that:
            \begin{itemize}
            \item $\gamma(t_0)=id_{t_0}$
            \item $t_0\succ{}t$
            \item $\mathtt{comp}(id_{t_0}$, $"transition"$,
              $gm_{t_0}$, $ipm_{t_0}$, $opm_{t_0})\in{}d.cs$
            \item ${<}\mathtt{fired\Rightarrow{}id_{ft_0}}{>}\in{}opm_{t_0}$
            \item ${<}\mathtt{output\_transitions\_fired(0)\Rightarrow{}id_{ft_0}}{>}\in{}ipm_{p}$
            \end{itemize}
