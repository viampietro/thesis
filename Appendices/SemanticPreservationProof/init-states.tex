...
\begin{niproof}
  ...
  Rewriting $\sigma_0(id_p)("sm")$ as
  $\sigma_0(id_p)("initial\_marking")$,
  \framebox{$\sigma_0(id_p)("initial\_marking")=s_0.M(p)$.}
...
\begin{lemma}[Null input token sum at initial state]
  ...
  \inithyps{} then
  $\forall{}p\in{}P,id_p\in{}Comps(\Delta)~s.t.~\gamma(p)=id_p$,
  $\sigma_0(id_p)("s\_input\_token\_sum")=0$.
\end{lemma}

\begin{niproof}
  Given a $p$ and an $id_p$ s.t. $\gamma(p)=id_p$, let us show that
  \fbox{$\sigma_0(id_p)("s\_input\_token\_sum")=0$.}
  ...
  \begin{equation}
    ...
    \sigma_0(id_p)("sits")=\sum\limits_{i=0}^{\Delta(id_p)("ian")-1}
    \begin{cases}
      \sigma_0(id_p)("iaw")[i]~\mathtt{if}~\sigma_0(id_p)("itf")[i]\\
      0~otherwise \\
    \end{cases}
  \end{equation}
  
  Rewriting the goal with Equation~\eqref{eq:sits-at-init-state}:\\
  \fbox{$\sum\limits_{i=0}^{\Delta(id_p)("ian")-1}\begin{cases}
      \sigma_0(id_p)("iaw")[i]~\mathtt{if}~\sigma_0(id_p)("itf")[i]\\
      0~otherwise \\
    \end{cases}=0$.}
  ...
  \item $input(p)=\emptyset$:
    ...
    By construction,
    ${<}$\texttt{input\_arcs\_number}$\Rightarrow{}1{>}\in{}gm_p$,
  ...
  ${<}$\texttt{input\_arcs\_number}$\Rightarrow{}1{>}\in{}gm_p$, we
  can deduce $\Delta(id_p)("ian")=1$.

  By property of the initialization relation, \InCsCompP,
  ${<}$\texttt{input\_transitions\_fired(0)}$\Rightarrow{}\mathtt{true}{>}\in{}ipm_p$
  and
  ${<}\mathtt{input\_arcs\_weights(0)\Rightarrow{}0}{>}\in{}ipm_p$, we
  can deduce $\sigma_0(id_p)("itf")[0]=\mathtt{true}$ and
  $\sigma_0(id_p)("iaw")[0]=0$.

  Rewriting the goal with $\Delta(id_p)("ian")=1$,
  $\sigma_0(id_p)("itf")[0]=\mathtt{true}$,
  $\sigma_0(id_p)("iaw")[0]=0$ and simplifying the goal,
  \qedbox{tautology.}
  
\item $input(p)\neq\emptyset$:

  By construction,
  ${<}\texttt{input\_arcs\_number}\Rightarrow{}\vert{}input(p)\vert{>}\in{}gm_p$,
  and by property of the elaboration relation, and \InCsCompP{}, we
  can deduce $\Delta(id_p)("ian")=\vert{}input(p)\vert$.
  ...
  \item \textbf{INDUCTION CASE}:
    \begin{ih}
      $\sum\limits_{i=1}^{\Delta(id_p)("ian")-1}\begin{cases}
        \sigma_0(id_p)("iaw")[i]~\mathtt{if}~\sigma_0(id_p)("itf")[i]\\
        0~otherwise \\
      \end{cases}=0$
    \end{ih}
    ...
    \fbox{$\begin{cases}
        \sigma_0(id_p)("iaw")[0]~\mathtt{if}~\sigma_0(id_p)("itf")[0]\\
        0~otherwise \\
      \end{cases}+\sum\limits_{i=1}^{\Delta(id_p)("ian")-1}\begin{cases}
        \sigma_0(id_p)("iaw")[i]~\mathtt{if}~\sigma_0(id_p)("itf")[i]\\
        0~otherwise \\
      \end{cases}=0$}
    ...
    $id_t\in{}Comps(\Delta),gm_t,ipm_t,opm_t$ s.t. \InCsCompT{},
    ...
    By property of the initialization relation, \InCsCompP{},
    \InCsCompT{}, ${<}\mathtt{fired\Rightarrow}id_{ft}{>}\in{}opm_t$
    and
    ${<}$\texttt{input\_transitions\_fired(0)}$\Rightarrow{}\mathtt{id_{ft}}{>}\in{}ipm_p$,
    we can deduce $\sigma_0(id_p)("itf")[0]=\sigma_0(id_t)("fired")$.

    Rewriting the goal with $\sigma_0(id_p)("itf")[0]=\sigma_0(id_t)("fired")$:\\
    \fbox{$\begin{cases}
        \sigma_0(id_p)("iaw")[0]~\mathtt{if}~\sigma_0(id_t)("fired")\\
        0~otherwise \\
      \end{cases}=0$}

    Appealing to Lemma~\ref{lem:init-states-fired-false}, we can
    deduce $\sigma_0(id_t)("fired")=\mathtt{false}$.

    Rewriting the goal with $\sigma_0(id_t)("fired")=\mathtt{false}$,
...
\begin{lemma}[Null output token sum at initial state]
  ...
  \inithyps{} then
  $\forall{}p\in{}P,id_p\in{}Comps(\Delta)~s.t.~\gamma(p)=id_p$,
  $\sigma_0(id_p)("s\_output\_token\_sum")=0$.
\end{lemma}
...
\begin{niproof}
  ...
  $s_0$,
  \framebox{$\sigma_0(id_t)("s\_reinit\_time\_counter")=\mathtt{false}$.}\\
  ...
  \framebox{$\prod\limits_{i=0}^{\Delta(id_t)("ian")-1}\sigma_0(id_t)("rt")[i]=\mathtt{false}$.}\\
  ...
  \item \textbf{CASE} $input(t)=\emptyset$.
    ...
    deduce $\sigma_0(id_t)("rt")[0]=\mathtt{false}$.

    Rewriting $\Delta(id_t)("ian")$ as $1$ and
    $\sigma_0(id_t)("rt")[0]$ as $\mathtt{false}$, \qedbox{tautology.}
    
  \item \textbf{CASE} $input(t)\neq\emptyset$.

    To prove the current goal, we can equivalently prove that\\
    \fbox{$\exists{}i\in[0,\Delta(id_t)("ian")-1]~s.t.~\sigma_0(id_t)("rt")[i]=\mathtt{false}$.}
    ...
    Then, we must show \framebox{$\sigma_0(id_t)("rt")[i]=\mathtt{false}$.}
    ...
    \framebox{$\sigma_p^0("rtt")[j]=\mathtt{false}$.}
    ...
    $j\in[0,\Delta(id_p)("oan")-1]$,
    \qedbox{$\sigma_0(id_p)("rtt")[j]=\mathtt{false}$.}
...
\begin{lemma}[No fired at initial state]
  ...
  $\forall{}d\in{}design,\Delta\in{}ElDesign(d,\mathcal{D}_\mathcal{H}),\sigma_e,\sigma_0\in\Sigma(\Delta),id_t\in{}Comps(\Delta),gm_t,ipm_t,opm_t$
  s.t. :
  \begin{itemize}
  ...
  \item $\mathtt{comp}(id_t$, $"transition"$, $gm_t$, $ipm_t$,
    $opm_t)\in{}d.cs$
  \end{itemize}
  then $\sigma_0(id_t)("fired")=\mathtt{false}$.
\end{lemma}

\begin{niproof}
  Assuming all the above hypotheses, let us show
  \fbox{$\sigma_0(id_t)("fired")=\mathtt{false}$.}
  ...
  \begin{equation}
    ...
    \sigma_0(id_t)("fired")=\sigma_0(id_t)("s\_firable")~.~\sigma_0(id_t)("s\_priority\_combination")
  \end{equation}

  Rewriting the goal with Equation~\eqref{eq:fired-at-init-state}:
  \fbox{$\sigma_0(id_t)("sfa")~.~\sigma_0(id_t)("spc")=\mathtt{false}$.}
  
  By property of the initialization relation, \InCsCompT, and through
  the examination of the \texttt{firable} process defined in the
  transition design architecture, we can deduce $\sigma_0(id_t)("sfa")=\mathtt{false}$.

  Rewriting the goal with $\sigma_0(id_t)("sfa")=\mathtt{false}$ and
