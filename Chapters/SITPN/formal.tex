Hoping that the reader has now a fair understanding of the concepts
underlying the SITPNs and their evolution dynamics, we now give the
formal definition of the SITPN structure and of its execution
semantics. We also introduce the concept of a \emph{well-defined}
SITPN at the end of the section.

\subsection{SITPN structure}
\label{sec:struct-and-wd}

\noindent{}An SITPN is formally defined as follows:
...
\begin{definition}[SITPN]
  ...
  \item $\mathcal{F}=\{f_0,\ldots,f_k\}$, a finite set of functions (discrete actions).
...
\begin{definition}[SITPN State]
  ...
  \item\label{item:sitpn-state-tc} $I\in{}T_i{}\rightarrow\mathbb{N}$
    ...
    counter value.
  \item\label{item:sitpn-state-rst} $reset_t\in{}T_i\rightarrow\mathbb{B}$ is the function mapping
    time transitions to time interval reset orders (defined as
    Booleans).
...
\emph{firable} and a \emph{fired} transition.

We use the two following notations to simplify the formalization of
the SITPN semantics.
...
\begin{notation}[Sum expressions and arc types]
  ...
  \item The first kind of expression computes a number of output
    ...
    This expression is a notation for
    $\sum\limits_{t\in{}T'}\begin{cases}\omega~if~pre(p,t)=(\omega,\mathtt{basic})\\
      0~otherwise \end{cases}$

    Indeed, when computing a sum of output tokens (i.e. resulting of a
    firing process), we want to add to the sum the weight of the arc
    between place $p$ and a transition $t\in{}T'$ only if there exists
    an arc of type $\mathtt{basic}$ from $p$ to $t$ (remember that the
    test and inhibitor never lead to the withdrawal of tokens during
    the firing process). Otherwise, we add 0 to the sum as it is a
    neutral element of the addition operator over natural numbers.
  ...
  \item The second kind expression computes a number of input tokens.
    ...
    Here, we add the weight of the arc
...
As explain in Section~\ref{subsec:hpn-particularities}, the firability
conditions are not sufficient for a transition to be fired. A
transition must also be enabled by the residual marking to go through
the firing process. Definition~\ref{def:fired} gives the formal
definition of a fired transition:
...
given transition $t$, the definition of the $Pr$ set of $t$ is all the
transitions with a higher firing priority than $t$ that are also fired
transitions; hence the recursive definition.

In Definition~\ref{def:fired}, the marking
$M-\sum_{t_i\in{}Pr(t)}pre(t_i)$ formally qualifies the residual
marking a SITPN state $s$ for a given transition $t$.
...
semantics as two construction cases parameterized by a clock event,
either falling edge or rising edge, and describes the evolution of the
state of a SITPN.
...
\begin{definition}[SITPN Semantics]
  ...
  \item $\rightarrow\subseteq{}S(sitpn)\times{}L\times{}S(sitpn)$ is the SITPN state
    ...
    \item $\forall\tau\in\mathbb{N}$, $\forall{}s,s'\in{}S(sitpn)$,
...
status of actions. Rules~\ref{it:reset-counters},
\ref{it:inc-counters}, \ref{it:locked-counters} and
\ref{it:reset-not-sens} focus on the update of time counter values.
In Rule~\ref{it:inc-counters} of the SITPN semantics, the
\emph{active} time counters refer to the time counters that have not
yet overreached the upper bound of the associated time interval. Of
course, a time counter is always active when the upper bound is
infinite. In Rule~\ref{it:locked-counters}, the \emph{locked} time
counters refer to the time counters that have overreached the upper
bound of the associated time interval. Of course, time counters can
never be locked if the presence of an infinite upper bound. In
Rules~\ref{it:inc-counters} and \ref{it:locked-counters}, for a given
time interval $i$, $upper(i)$ denotes the upper bound of the time
interval, and $lower(i)$ denotes the lower bound of the time interval.
...
obtained after the consumption phase of the firing process.
...
describes the state of the SITPN before a rising edge. Judging by the
current SITPN state at \circled{1}, transition $t_0$ and $t_2$ will be
fired on the next clock event. Situation~\circled{2} depicts the
marking obtained after the consumption phase of the firing process,
i.e. the so-called \emph{transient} marking. Situation~\circled{3}
corresponds to the marking at the end of the firing process. At
...
to the execution trace, i.e. an ordered list of states, that it
...
three ways of mutual exclusion, namely: mutual exclusion with
complementary conditions, mutual exclusion with disjoint time
intervals and mutual exclusion with inhibitor arcs. Here, we give the
formal definition of these three means of mutual exclusion.
...
\begin{definition}[Mutual exclusion with an inhibitor arc]
  \label{def:mutex-conds} Given two conflicting transitions $t_0$ and
...
Figure~\ref{fig:mutex} illustrates the three means of mutual exclusion
that can be applied to solve a conflict between two transitions.

\begin{figure}[H]
  \centering
  \includegraphics[keepaspectratio,width=.8\linewidth]{Figures/SITPN/mutex}
  \caption[Examples of conflicting transitions in mutual exclusion.]{
    ...
    \circled{1}, an example of mutual exclusion with disjoint time
    intervals; at \circled{2}, an example of mutual exclusion with
    complementary conditions; at \circled{3}, an example of mutual
    exclusion with an inhibitor arc.}
  \label{fig:mutex}
\end{figure}

In Figure~\ref{fig:mutex}, in situation \circled{2}, condition $c_1$
is associated to $t_1$ and the complementary condition is associated
to $t_0$ thus creating the mutual exclusion. In situation \circled{3},
the arc $(p_0,t_0)$ and $(p_0,t_1)$ ensure the mutual exclusion
...
\begin{definition}[Well-defined SITPN]\label{def:wd-sitpn}
  ...
  \item For all conflict group as defined in
    ...
    pair of transitions in the conflict group) must be solved by one
    of the mean of mutual exclusion, or, the priority relation must be
    a strict total order over the transitions of the conflict group.
  \end{itemize}
\end{definition}
