...
Petri nets (PNs), invented by C. A. Petri \cite{Petri1962}, are used
to model a broad range of \emph{dynamic} systems: resource sharing
between concurrent processes \cite{David1994}, behavior of agents in
...
sensitized because there are no tokens in its input place $p_2$.
...
many tokens as the weight of the arcs) and produced in its output
places (as many tokens as the weight of the arcs).  This process
represents the occurrence of an event --denoted by the transition--
...
the transition firing process in the standard PN semantics is an
...
Interpreted Petri nets (IPN) \cite{David1994} describe the interaction
between a system and its outside environment. This class of PN
...
environment. Figure~\ref{fig:ipn} illustrates the use of actions,
