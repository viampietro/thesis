...
Step~2 to Step~3, the action activation status are updated; $a_0$
stays activated as place $p_0$ is still marked; $a_1$ becomes newly
activated as $p_1$ is marked. The time counter values are updated;
$t_0$'s time counter is set to zero as the transition previously
received a reset order. However, as $t_0$ is still sensitized by the
new marking, its time counter is incremented. Thus, the resulting time
counter value at Step~3 is of one (i.e. result of reset plus
incrementation). Also, the condition values are retrieved from the
environment. As a consequence, condition $c_0$ takes the value
\texttt{false}.
...
\texttt{false}. Thus, $t_1$ is not fired. The residual marking for
...
SITPNs inherit both the properties of time PNs and interpreted
PNs. The phenomenon of \emph{locked} time counters is a consequence of
this combination. As illustrated in Figure~\ref{fig:locked-tc}, the
value of a time counter can overreach the upper bound of its
associated time interval. This situation can only arise if a condition
hinders the firing of a given transition while the considered
transition is still enabled by the marking. As a consequence, the time
counter will be incremented at every clock cycle until the upper bound
of the time interval is overreached. Then, at this point, the time
counter is said to be \emph{locked} and its value will no more evolve.
