...
which values are set by the environment, then we can consider that
conditions directly receive their value from the environment; we no
more have to consider the Boolean expressions associated with
conditions. Regarding actions and functions, we are only interested in
the fact that a given action/function is activated/executed but no
more in actually executing the associated operation.
