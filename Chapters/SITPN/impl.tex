...
We use lists of natural numbers, i.e. \coqe|list nat| in \coq{}, to
...
\coqe|Sitpn| record. To be able to use these finite sets in the
...
$M_0\in{}P\rightarrow\mathbb{N}$, we use the \coq{} \coqe|sig| type to
define subsets of elements verifying a certain property. Thus, we
define the finite set $P$ as the subset of natural numbers that are
members of the \coqe|places| list (Line~\ref{line:P}). We use the
\coqe|In| proposition of the \coq{} standard library to express the
membership of a natural number in the \coqe|places| list. Also, the
\coqe|ArcT| type (Line~\ref{line:pre}) implements the set
$\{\mathtt{inhib},\mathtt{test},\mathtt{basic}\}$, the
\coqe|TimeInterval| type (Line~\ref{line:Is}) implements the set
$\mathbb{I}^{+}$ of time intervals, and the \coqe|MOneZeroOne| type
(Line~\ref{line:mone}) implements the set $\{0,1,-1\}$. The priority
relation is implemented by the \coqe|pr| function (Line~\ref{line:pr})
taking two transitions in parameter and projecting to the type of
logical propositions, i.e. the \coqe|Prop| type.
...
The \coqe|SitpnState| type definition depends on a given SITPN passed
as a parameter; it is an example of dependent type. Then, we can refer
to the attributes of \coqe|sitpn| in the definition of the
\coqe|SitpnState| structure.  Projection functions are automatically
generated to access the attributes of a record at the declaration of a
type with the \coqe|Record| keyword.  Thus, in
Listing~\ref{lst:sitpn-state-impl}, we can refer to the set of places
of \coqe|sitpn| with the term \coqe|P sitpn|. The term \coqe|$T_i$ sitpn|
denotes the set of time transitions of \coqe|sitpn|. The set of
time transitions for a given SITPN is declared as a \coqe|sig| type
qualifying to the subset of transitions with an associated time
interval.
...
to the upper bound of the associated time interval is implemented by a
separate predicate \texttt{TcLeUpper}. Similarly, the
\texttt{TcGtUpper} predicate implements the inverse test.
