...
% \begin{figure}[H]
%   {\fontsize{8}{11}\selectfont\textsc{WFTypedefEnv}}
...
% \begin{figure}[H]
%   {\fontsize{8}{11}\selectfont\textsc{WFDesign}}
...
% \begin{figure}[H]
%   {\fontsize{8}{11}\selectfont\textsc{WFGammaDeltaEnv}}
%   \vspace{2ex}
...
% \begin{figure}[H]
%   {\fontsize{8}{11}\selectfont\textsc{WFGammaDeltaLambdaEnv}}
%   \vspace{2ex}
...
decomposed into simulation cycles.  Each simulation cycle is divided
in four parts entailed by the \emph{synchronous} execution of
...
\begin{table}[H]
  {\fontsize{8}{11}\selectfont\textsc{FullSim}}
  ...
  \begin{prooftree}[template=\inserttext]
    ...
    \infer3 [] { $\mathcal{D},\Delta,\mathcal{M}_g,E_p,\tau\vdash$
      $\mathrm{d}\xrightarrow{full}\theta$ }
  \end{prooftree}
\end{table}
...
\begin{table}[H]
  ...
  \begin{tabular}{@{}l}
    ...
    {\fontsize{8}{11}\selectfont\textsc{SimEnd}} \\
  ...
  \begin{tabular}{l}
    ...
    {\fontsize{8}{11}\selectfont\textsc{SimLoop}} \\
    
    {\begin{prooftree}[template=\inserttext]
        
        % First column.
        \hypo{$\mathcal{D},E_p,\Delta,\tau,\sigma\vdash{}cs\xrightarrow{\uparrow,\downarrow}\sigma'$}
        ...
        [$T_c>0$]
        {
          $\mathcal{D},E_p,\Delta,\tau,\sigma\vdash{}cs\rightarrow\theta$
        }
      \end{prooftree}} \\
  \end{tabular}
...
each clock event. Fresh input port values are coming from the
environment $E_p$. The updates are made in the definitions of
$\sigma_{injr}$ and $\sigma_{injf}$. These definitions are expressed
as side conditions.

\begin{table}[H]
  {\fontsize{8}{11}\selectfont\textsc{SimCyc}}
  
  \begin{prooftree}[template=\inserttext]

    % Rising
    \hypo{$\mathcal{D},\Delta,\sigma_{injr}\vdash\mathrm{cs}\xrightarrow{\uparrow}\sigma_r$}

    % Falling
    \infer[no rule]1{$\mathcal{D},\Delta,\sigma_{injf}\vdash\mathrm{cs}\xrightarrow{\downarrow}\sigma_f$}

    % Stabilize after rising.
    \hypo{$\mathcal{D},\Delta,\sigma_r\vdash\mathrm{cs}\xrightarrow{\rightsquigarrow}\sigma'$}

    % Stabilize after falling.
    \infer[no rule]1{$\mathcal{D},\Delta,\sigma_f\vdash\mathrm{cs}\xrightarrow{\rightsquigarrow}\sigma''$}
    ...
    [{
      ...
      \begin{tabular}{@{}l}
        ...
        $\sigma_{injr}={<}\mathcal{S}\ocup{}E_p(\tau,\uparrow),
        \mathcal{C},
        \mathcal{E}\cup(\mathcal{S}\dcap{}E_p(\tau,\uparrow)){>}$ \\
        $\sigma'={<}\mathcal{S}', \mathcal{C}', \mathcal{E}'{>}$ \\
        $\sigma_{injf}={<}\mathcal{S}'\ocup{}E_p(\tau,\downarrow),
        \mathcal{C}',
        \mathcal{E}'\cup(\mathcal{S}'\dcap{}E_p(\tau,\downarrow)){>}$ \\
      \end{tabular}
    }] {
...
\begin{remark}[Input ports and signal store]
  ...
  $E_p\in{}\mathbb{N}\rightarrow{}Clk\rightarrow{}(input\mhyphen{}id\rightarrow{}value)\mathbb{}$,
  $\tau\in\mathbb{N}$, $clk\in{}Clk$, we have
  $\mathtt{dom}(E_p(\tau,clk))\subset{}\mathtt{dom}(\mathcal{S})$,
  ...
  signals constitutes the domain of state $\sigma$'s signal store
  (i.e, $\mathcal{S}$); thus we have
  $\mathtt{dom}(E_p(\tau,clk))\subset{}\mathtt{dom}(\sigma.\mathcal{S})$.
  Consequently, all signals that belong to
  $\mathtt{dom}(E_p(\tau,clk))$ and have a different value than in the
  signal store $\mathcal{S}$ are captured by the expression
  $\mathcal{S}\dcap{}E_p(\tau,clk)$.
\end{remark}

\subsection{Initialization rules}

\begin{table}[H]
  {\fontsize{8}{11}\selectfont\textsc{Init}}
...
\begin{table}[H]
  \centering
  \begin{tabular}{@{}l}
    {\fontsize{8}{11}\selectfont\textsc{PsRunInit}} \\
    {\begin{prooftree}[template=\inserttext]
        
        % Runs seq.
        \hypo{$\Delta,NoEv(\sigma),\Lambda\vdash\mathrm{ss}\xrightarrow{ss_i}{}\sigma',\Lambda'$}
...
\label{subsubsec:comp-stmt-init}
...
\begin{table}[H]
  \centering
  \begin{tabular}{@{}l}
    {\fontsize{8}{11}\selectfont\textsc{CompRunInitEvents}} \\
    {\begin{prooftree}
        
        % Builds mapping for in ports.
        \hypo{\Delta,\Delta_c,\sigma,\sigma_c&\vdash\mathrm{ipmap}\xrightarrow{mapip}\sigma'_c}
        ...
        [{\renewcommand{\arraystretch}{1.5}
          \begin{tabular}{@{}l}
            ...
            $\sigma_c''={<}\mathcal{S}_c'',\mathcal{C}_c'',\mathcal{E}_c''{>}$, $\mathcal{E}_c''\neq\emptyset$ \\
            $\sigma''=(\sigma'(\mathrm{id}_c)\leftarrow\sigma_c'')\cup_\mathcal{E}\{\mathrm{id}_c\}$ \\
          \end{tabular}
        }] {
          \mathcal{D},\Delta,\sigma\vdash~
          $\vhdle|comp|$~(\mathrm{id}_c, \mathrm{id}_e, \mathrm{gmap},
          \mathrm{ipmap}, \mathrm{opmap})
          \xrightarrow{runinit}{}\sigma''
        }
      \end{prooftree}} \\
  \end{tabular}
...
\begin{table}[H]
  \centering
  \begin{tabular}{@{}l}
    {\fontsize{8}{11}\selectfont\textsc{CompRunInitNoEvent}} \\
    {\begin{prooftree}
        ...
        [{\renewcommand{\arraystretch}{1.5}
          \begin{tabular}{@{}l}
            $id_e\in\mathcal{D}$ \\
            $\Delta(\mathrm{id}_c)=\Delta_c$, $\sigma(\mathrm{id}_c)=\sigma_c$ \\
            $\sigma_c''={<}\mathcal{S}_c'',\mathcal{C}_c'',\emptyset{>}$ \\
          \end{tabular}
        }] {
          \mathcal{D},\Delta,\sigma\vdash~
          $\vhdle|comp|$~(\mathrm{id}_c, \mathrm{id}_e, \mathrm{gmap},
...
\begin{remark}[No event on component execution]
  If, after the injection of fresh input values and the execution of
  its internal behavior, a component instance displays an empty set of
  events, then its state stayed the same
  ($\sigma_c=\sigma'_c=\sigma''_c$). Then, no need to override the
  component instance state in the embedding design state, and no need
  to propagate the values coming from the output ports of the
  component instance to the embedding design.
\end{remark}
...
\begin{table}[H]
  \begin{tabular}{l}
    {\fontsize{8}{11}\selectfont\textsc{ParRunInit}} \\
    {\begin{prooftree}
        \hypo{\mathcal{D},\Delta,\sigma\vdash\mathrm{cs}\xrightarrow{runinit}\sigma'}
        \hypo{\mathcal{D},\Delta,\sigma\vdash\mathrm{cs'}\xrightarrow{runinit}\sigma''}
        \infer2
  ...
  \end{tabular}
  \begin{tabular}{l}
    {\fontsize{8}{11}\selectfont\textsc{NullRunInit}} \\
    {\begin{prooftree}
        \infer0
        {
...
\begin{table}[H]
  {\fontsize{8}{11}\selectfont\textsc{PsRENoClk}}
...
\begin{table}[H]
  {\fontsize{8}{11}\selectfont\textsc{PsREClk}}
  
  \begin{prooftree}[template=\inserttext]

    \hypo{$\Delta,\sigma,\Lambda\vdash\mathrm{ss}\xrightarrow{ss_\uparrow}{}\sigma'$}
...
\begin{table}[H]
  {\fontsize{8}{11}\selectfont\textsc{PsFENoClk}}
...
\begin{table}[H]
  {\fontsize{8}{11}\selectfont\textsc{PsFEClk}}
  
  \begin{prooftree}[template=\inserttext]

    \hypo{$\Delta,\sigma,\Lambda\vdash\mathrm{ss}\xrightarrow{ss_\downarrow}{}\sigma'$}
...
\begin{table}[H]
  \centering
  \begin{tabular}{@{}l}
    {\fontsize{8}{11}\selectfont\textsc{CompREEvents}} \\
    {\begin{prooftree}[template=\inserttext]
        ...
        \infer[no rule]1{$\mathcal{D},\Delta_c,\sigma_c'\vdash{}\mathcal{D}(id_e).cs\xrightarrow{\uparrow}\sigma_c''$}
        ...
        [{\renewcommand{\arraystretch}{1.5}
          \begin{tabular}{@{}l}
            $id_e\in\mathcal{D}$ \\
            $\Delta(\mathrm{id}_c)=\Delta_c$, $\sigma(\mathrm{id}_c)=\sigma_c$ \\
            $\sigma''_c={<}\mathcal{S}''_c,\mathcal{C}''_c,\mathcal{E}''_c{>}$, $\mathcal{E}''_c\neq\emptyset$ \\
            $\sigma''=(\sigma'(\mathrm{id}_c)\leftarrow\sigma_c'')\cup_\mathcal{E}\{\mathrm{id}_c\}$ \\
          \end{tabular}
        }]
        {
          $\mathcal{D},\Delta,\sigma\vdash$
          \vhdle|comp| (id$_c$, id$_e$, gmap, ipmap, opmap)
...
\begin{table}[H]
  \centering
  \begin{tabular}{@{}l}
    {\fontsize{8}{11}\selectfont\textsc{CompRENoEvent}} \\
    {\begin{prooftree}[template=\inserttext]
        ...
        [{\renewcommand{\arraystretch}{1.5}
          \begin{tabular}{@{}l}
            $id_e\in\mathcal{D}$ \\
            $\Delta(\mathrm{id}_c)=\Delta_c$, $\sigma(\mathrm{id}_c)=\sigma_c$ \\
            $\sigma''_c={<}\mathcal{S}''_c,\mathcal{C}''_c,\emptyset{>}$ \\
          \end{tabular}
        }]
        {
          $\mathcal{D},\Delta,\sigma\vdash$
...
\begin{table}[H]
  \centering
  \begin{tabular}{@{}l}
    {\fontsize{8}{11}\selectfont\textsc{CompFEEvents}} \\
    {\begin{prooftree}[template=\inserttext]
        ...
        % Executes rising on component behavior.
        \infer[no rule]1{$\mathcal{D},\Delta_c,\sigma_c'\vdash{}\mathcal{D}(id_e).cs\xrightarrow{\downarrow}\sigma_c''$}
...
\begin{table}[H]
  \centering
  \begin{tabular}{@{}l}
    {\fontsize{8}{11}\selectfont\textsc{CompFENoEvent}} \\
    {\begin{prooftree}[template=\inserttext]
...
\begin{sideconds}
  \begin{itemize}
  \item As the set of events of the source state $\sigma$ is not
    emptied during the evaluation of concurrent statements at clock
    phases, we have $\mathcal{E}\subseteq{}\mathcal{E'}$ and
    $\mathcal{E}\subseteq{}\mathcal{E''}$. Thus, the side condition
    $\mathcal{E'}\cap\mathcal{E''}=\emptyset$, used to prevent
    multiply driven signals in the evaluation rules of the
    initialization and stabilization phases, only holds here if
    $\mathcal{E}=\emptyset$, which is not always the case. So, we have
    to remove the set $\mathcal{E}$ from the intersection of
    $\mathcal{E'}$ and $\mathcal{E''}$ before checking that there are
    no multiply-driven signals:
    $(\mathcal{E}'\cap\mathcal{E}'')\setminus\mathcal{E}=\emptyset$.
  \end{itemize}
\end{sideconds}

\begin{table}[H]
  \begin{tabular}{l}
    {\fontsize{8}{11}\selectfont\textsc{ParFE}} \\
    {\begin{prooftree}
  ...
  \end{tabular}
  \begin{tabular}{l}
    {\fontsize{8}{11}\selectfont\textsc{NullFE}} \\
    {\begin{prooftree}
        \infer0
        {
...
\begin{table}[H]
  \begin{tabular}{l}
    {\fontsize{8}{11}\selectfont\textsc{ParRE}} \\
    {\begin{prooftree}
  ...
  \end{tabular}
  \begin{tabular}{l}
    {\fontsize{8}{11}\selectfont\textsc{NullRE}} \\
    {\begin{prooftree}
        \infer0
...
\begin{remark}[Events at clock phases]
  Considering that an $\uparrow$ (or $\downarrow$) phase, is always
  preceded by a stabilization phase during the full simulation of an
  \hvhdl{} design, we are always able to state that the set of events
  of source state $\sigma$ is a subset of input port identifiers, i.e
  $\mathcal{E}\subseteq{}Ins(\Delta)$. Thus, withdrawing the set
  $\mathcal{E}$ from $\mathcal{E}'\cap\mathcal{E}''$ will never
  overshadow cases of multiply-driven signals. Indeed, only input port
  identifiers will be withdrawn from the set by the minus
  operation. And, input port identifiers are not subject to the
  problem of multiply-driven signals, because they are read-only
  constructs.
\end{remark}
...
\begin{table}[H]
  \centering
  \begin{tabular}{@{}l}
    {\fontsize{8}{11}\selectfont\textsc{StabilizeEnd}} \\
    
    {\begin{prooftree}[template=\inserttext]
        \infer0
        [$\sigma={<}\mathcal{S},\mathcal{C},\emptyset{>}$]
        {
  ...
  \end{tabular}
  \begin{tabular}{@{}l}
    {\fontsize{8}{11}\selectfont\textsc{StabilizeLoop}} \\
    {\begin{prooftree}[template=\inserttext]
...
\begin{table}[H]
  {\fontsize{8}{11}\selectfont\textsc{PsCombStable}}
...
\begin{table}[H]
  {\fontsize{8}{11}\selectfont\textsc{PsCombUnstable}}
  
  \begin{prooftree}[template=\inserttext]
    
    % Executes ss.
    \hypo{$\Delta,NoEv(\sigma),\Lambda\vdash\mathrm{ss}\xrightarrow{ss_c}{}\sigma'$}
...
\begin{table}[H]
  \centering
  \begin{tabular}{@{}l}
    {\fontsize{8}{11}\selectfont\textsc{CompCombEvents}} \\
    {\begin{prooftree}[template=\inserttext]
        ...
        \infer[no rule]1{$\mathcal{D},\Delta_c,\sigma_c'\vdash{}\mathcal{D}(id_e).cs\xrightarrow{comb}\sigma_c''$}
...
\begin{table}[H]
  \centering
  \begin{tabular}{@{}l}
    {\fontsize{8}{11}\selectfont\textsc{CompCombNoEvent}} \\
    {\begin{prooftree}[template=\inserttext]
...
\begin{table}[H]
  \begin{tabular}{l}
    {\fontsize{8}{11}\selectfont\textsc{ParComb}} \\
    {\begin{prooftree}
        \hypo{\mathcal{D},\Delta,\sigma\vdash\mathrm{cs}\xrightarrow{comb}\sigma'}
        \hypo{\mathcal{D},\Delta,\sigma\vdash\mathrm{cs'}\xrightarrow{comb}\sigma''}
        \infer2
  ...
  \end{tabular}
  \begin{tabular}{l}
    {\fontsize{8}{11}\selectfont\textsc{NullComb}} \\
    {\begin{prooftree}
        \infer0
        {
...
\begin{table}[H]
  \begin{tabular}{@{}l}
    {\fontsize{8}{11}\selectfont\textsc{MapipSimple}} \\
    {\begin{prooftree}
...
\begin{table}[H]
  \begin{tabular}{@{}l}
    {\fontsize{8}{11}\selectfont\textsc{MapipPartial}} \\
    {\begin{prooftree}
...
\begin{table}[H]
  {\fontsize{8}{11}\selectfont\textsc{MapipComp}}
...
\begin{table}[H]
  \begin{tabular}{@{}l}
    {\fontsize{8}{11}\selectfont\textsc{MapopOpen}} \\
    {\begin{prooftree}
...
\begin{table}[H]
  \begin{tabular}{@{}l}
    {\fontsize{8}{11}\selectfont\textsc{MapopSimpleToSimple}} \\
    {\begin{prooftree}
...
\begin{table}[H]
  \begin{tabular}{@{}l}
    {\fontsize{8}{11}\selectfont\textsc{MapopSimpleToPartial}} \\
    {\begin{prooftree}
...
\begin{table}[H]
  \begin{tabular}{@{}l}
    {\fontsize{8}{11}\selectfont\textsc{MapopPartialToSimple}} \\
    {\begin{prooftree}
...
\begin{table}[H]
  \begin{tabular}{@{}l}
    {\fontsize{8}{11}\selectfont\textsc{MapopPartialToPartial}} \\
    {\begin{prooftree}
...
\begin{table}[H]
  {\fontsize{8}{11}\selectfont\textsc{MapopComp}}
...
\begin{table}[H]
  \centering
  \begin{tabular}{@{}l}
    {\fontsize{8}{11}\selectfont\textsc{OutO}} \\
    {\begin{prooftree}
        \infer0
        [{
          \begin{tabular}{@{}l}
            $\mathrm{id}_s\in{}Outs(\Delta_c)$ \\
            $\mathrm{id}_s\in\sigma_c$ \\
          \end{tabular}
        }] {
          \Delta_c,\sigma_c\vdash
          \mathrm{id}_s
          \xrightarrow{e_o}
          \sigma_c(\mathrm{id}_s)
        }
      \end{prooftree}} \\
  \end{tabular}
\end{table}
...
\begin{table}[H]
  \centering
  \begin{tabular}{@{}l}
    {\fontsize{8}{11}\selectfont\textsc{IdxOutO}} \\
    {\begin{prooftree}
        ...
        }]
        {
          ...
          \xrightarrow{e_o}
          \mathtt{get\_at}(i,\sigma_c(\mathrm{id}_s))
        }
      \end{prooftree}} \\
  \end{tabular}
\end{table}
...
generates a new state $\sigma'$ with a modified signal value store
$\mathcal{S'}$ and a new event set $\mathcal{E'}$.

\begin{table}[H]
  \begin{tabular}{@{}l}
    {\fontsize{8}{11}\selectfont\textsc{SigAssign}} \\
    {\begin{prooftree}
        ...
        }]
        {
          ...
          \xrightarrow{ss}
          {<}\mathcal{S}',\mathcal{C},\mathcal{E}'{>},\Lambda
        }
      \end{prooftree}} \\
  \end{tabular}
\end{table}
...
\begin{table}[H]
  \begin{tabular}{@{}l}
    {\fontsize{8}{11}\selectfont\textsc{IdxSigAssign}} \\
    {\begin{prooftree}
...
\begin{table}[H]
  \begin{tabular}{@{}l}
    {\fontsize{8}{11}\selectfont\textsc{VarAssign}} \\    
    {\begin{prooftree}
        ...
        }]
        {
          ...
          \xrightarrow{ss}
          \sigma,\Lambda(id_v)\leftarrow{}(T,v)
        }
      \end{prooftree}} \\
  \end{tabular}
\end{table}
...
\begin{table}[H]
  \begin{tabular}{@{}l}
    {\fontsize{8}{11}\selectfont\textsc{IdxVarAssign}} \\    
    {\begin{prooftree}
        ...
        }]
        {
          ...
          \xrightarrow{ss}
          \sigma,\Lambda(id_v)\leftarrow{}(T,\mathtt{set\_at}(v,v_i,val))
        }
      \end{prooftree}} \\
  \end{tabular}
\end{table}
...
\begin{table}[H]
  \begin{tabular}{@{}l}
    {\fontsize{8}{11}\selectfont\textsc{If}$\top$} \\    
    {\begin{prooftree}
        ...
        % Evaluates ss.
        \hypo{\Delta,\sigma,\Lambda\vdash\mathrm{ss}\xrightarrow{ss}\sigma',\Lambda'}
  ...
  \end{tabular}
  \begin{tabular}{@{}l}
    {\fontsize{8}{11}\selectfont\textsc{If}$\bot$} \\
    {\begin{prooftree}
...
\begin{table}[H]
  \begin{tabular}{@{}l}
    {\fontsize{8}{11}\selectfont\textsc{IfElse}$\top$} \\    
    {\begin{prooftree}
  ...
  \end{tabular}
  \begin{tabular}{@{}l}
    {\fontsize{8}{11}\selectfont\textsc{IfElse}$\bot$} \\  
    {\begin{prooftree}
...
\begin{table}[H]
  \begin{tabular}{@{}l}
    {\fontsize{8}{11}\selectfont\textsc{Loop}$\bot$} \\
    {\begin{prooftree}
        ...
        \hypo{\Delta,\sigma,\Lambda_i&\vdash\mathrm{ss}\xrightarrow{ss}\sigma',\Lambda'}
        \infer[no rule]1{
          \Delta,\sigma',\Lambda'&\vdash~
          $\vhdle|for|$~(\mathrm{id}_v,\mathrm{e},\mathrm{e}')~\mathrm{ss}
          \xrightarrow{ss}\sigma'',\Lambda''
        }
...
\begin{table}[H]
  \begin{tabular}{@{}l}
    {\fontsize{8}{11}\selectfont\textsc{Loop}$\top$} \\    
    {\begin{prooftree}
        ...
        }]
        {
          ...
          $\vhdle|for|$~(\mathrm{id}_v,\mathrm{e},\mathrm{e}')~\mathrm{ss}
          \xrightarrow{ss}
          \sigma,\Lambda\setminus(id_v,\Lambda(id_v))
        }
      \end{prooftree}} \\
  \end{tabular}
\end{table}
...
\begin{table}[H]
  \begin{tabular}{@{}l}
    {\fontsize{8}{11}\selectfont\textsc{LoopInit}} \\
    {\begin{prooftree}
        ...
        % Upper bound check true.
        \hypo{\Delta,\sigma,\Lambda_i\vdash~
          $\vhdle|for|$~(\mathrm{id}_v,\mathrm{e},\mathrm{e}')~\mathrm{ss}
          \xrightarrow{ss}
          \sigma',\Lambda'}
...
\begin{table}[H]
  \begin{tabular}{l}
    {\fontsize{8}{11}\selectfont\textsc{RisingEdgeDefault}} \\    
    {\begin{prooftree}
  ...
  \end{tabular}
  \begin{tabular}{l}
    {\fontsize{8}{11}\selectfont\textsc{FallingEdgeDefault}} \\    
    {\begin{prooftree}
...
\begin{table}[H]
  \begin{tabular}{l}
    {\fontsize{8}{11}\selectfont\textsc{RisingEdgeExec}} \\    
    {\begin{prooftree}

        % Evaluates ss.
        \hypo{\Delta,\sigma,\Lambda\vdash\mathrm{ss}\xrightarrow{ss_\uparrow}\sigma',\Lambda'}
  ...
  \end{tabular}
  \begin{tabular}{l}
    {\fontsize{8}{11}\selectfont\textsc{FallingEdgeExec}} \\    
    {\begin{prooftree}

        % Evaluates ss.
        \hypo{\Delta,\sigma,\Lambda\vdash\mathrm{ss}\xrightarrow{ss_\downarrow}\sigma',\Lambda'}
...
\begin{table}[H]
  \begin{tabular}{l}
    {\fontsize{8}{11}\selectfont\textsc{RstDefault}} \\    
    {\begin{prooftree}

        % Evaluates ss.
        \hypo{\Delta,\sigma,\Lambda\vdash\mathrm{ss'}\xrightarrow{ss_f}\sigma',\Lambda'}
  ...
  \end{tabular}
  \begin{tabular}{l}
    {\fontsize{8}{11}\selectfont\textsc{RstExec}} \\    
    {\begin{prooftree}

        % Evaluates ss.
        \hypo{\Delta,\sigma,\Lambda\vdash\mathrm{ss}\xrightarrow{ss_i}\sigma',\Lambda'}
...
\begin{table}[H]
  \begin{tabular}{l}
    {\fontsize{8}{11}\selectfont\textsc{SeqStmt}} \\
    {\begin{prooftree}
        ...
        % Conclusion.
        \infer2{
          \Delta,\sigma,\Lambda\vdash\mathrm{ss}\mathtt{;}~\mathrm{ss}'\xrightarrow{ss}\sigma'',\Lambda''
        }
      \end{prooftree}} \\
  \end{tabular}
  \begin{tabular}{l}
    {\fontsize{8}{11}\selectfont\textsc{NullStmt}} \\
    {\begin{prooftree}        

        % Conclusion.
        \infer0{
          \Delta,\sigma,\Lambda\vdash\mathtt{null}\xrightarrow{ss}\sigma,\Lambda
        }
      \end{prooftree}} \\
  \end{tabular}
\end{table}
...
\begin{table}[H]
  \begin{tabular}{l}
    {\fontsize{8}{11}\selectfont\textsc{Nat}} \\
    {\begin{prooftree}
        \infer0
        [{
          \begin{tabular}{@{}l}
  ...
  \end{tabular}
  \begin{tabular}{l}
    {\fontsize{8}{11}\selectfont\textsc{False}} \\
    {\begin{prooftree}
        \infer0
        {
  ...
  \end{tabular}
  \begin{tabular}{l}
    {\fontsize{8}{11}\selectfont\textsc{True}} \\
    {\begin{prooftree}
        \infer0
        {
...
\begin{table}[H]
  \centering
  \begin{tabular}{l}
    {\fontsize{8}{11}\selectfont\textsc{Aggreg}} \\
    {\begin{prooftree}
...
\begin{table}[H]
  \centering
  \begin{tabular}{l}
    {\fontsize{8}{11}\selectfont\textsc{Sig}} \\
    {\begin{prooftree}
        \infer0
        [{
          \begin{tabular}{@{}l}
  ...
  \end{tabular}
  \begin{tabular}{@{}l}
    {\fontsize{8}{11}\selectfont\textsc{Var}} \\
    {\begin{prooftree}
        \infer0
        [{
          \begin{tabular}{@{}l}
...
\begin{table}[H]
  \centering
  \begin{tabular}{@{}l}
    {\fontsize{8}{11}\selectfont\textsc{Gen}} \\
    {\begin{prooftree}
      \infer0
      [{
        \begin{tabular}{@{}l}
  ...
  \begin{tabular}{@{}l}
    {\fontsize{8}{11}\selectfont\textsc{IdxSig}} \\
    {\begin{prooftree}
...
\begin{table}[H]
  \centering
  \begin{tabular}{@{}l}
    {\fontsize{8}{11}\selectfont\textsc{IdxVar}} \\
    {\begin{prooftree}
...
\begin{table}[H]
  \begin{tabular}{@{}l}
    {\fontsize{8}{11}\selectfont\textsc{NatAdd}} \\
    {\begin{prooftree}
...
\begin{table}[H]
  \begin{tabular}{@{}l}
    {\fontsize{8}{11}\selectfont\textsc{NatSub}} \\
    {\begin{prooftree}
...
\begin{table}[H]
  \begin{tabular}{@{}l}
    {\fontsize{8}{11}\selectfont\textsc{OrdOp}} \\
...
\begin{table}[H]
  \begin{tabular}{@{}l}
    {\fontsize{8}{11}\selectfont\textsc{BoolBinOp}} \\
    {\begin{prooftree}
  ...
  \end{tabular}
  \begin{tabular}{@{}l}
    {\fontsize{8}{11}\selectfont\textsc{NotOp}} \\    
    {\begin{prooftree}
...
\begin{table}[H]
  {\fontsize{8}{11}\selectfont\textsc{EqOp}}
...
\begin{table}[H]
  {\fontsize{8}{11}\selectfont\textsc{DiffOp}}
...
\begin{table}[H]
  {\fontsize{8}{11}\selectfont\textsc{Parenth}}
