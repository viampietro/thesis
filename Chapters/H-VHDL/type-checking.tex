...
\begin{equation*}
  ...
  \forall{}\mathrm{id}_p\in\mathtt{dom}(Ports),~\mathrm{id}_p\in\mathcal{L}\vee
  (Ports(\mathrm{id}_p)=\mathtt{array}(T,n,m)\wedge\forall{}i\in[n,m],~(\mathrm{id}_p,i)\in\mathcal{L})
\end{equation*}
...
\begin{remark}[Unconnected output port.]
  ...
  connected while others are not (error case in \cite[p.7]{VHDL00}).
\end{remark}
