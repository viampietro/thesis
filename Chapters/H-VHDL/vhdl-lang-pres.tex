...
The LRM gives an informal semantics to VHDL designs through the
definition of a simulation algorithm. The purpose of simulation is to
compute the evolution of the values of signals during a certain time
interval.  Even though, there are two ways to consider a VHDL design,
i.e. a synthesis-oriented way and a simulation-oriented way, the LRM
does not define a synthesis-oriented semantics for the VHDL
language. Also, the community of VHDL users does not seem to have
agreed on a common synthesis-oriented semantics for the
language. Unlike the Verilog language \cite{Verilog2005}, no standards
exist.

Former to the simulation, the LRM defines an elaboration phase for the
design that will be simulated. The elaboration phase has several
goals. First, it builds the simulation environment and the design
simulation state. The simulation environment is built based on the
declarative parts of the top-level design; it maps the signals to
their types. In the design simulation state, each signal is associated
with a current value and with a driver. Driver maps time points to the
value of a signal at this time point. The association between a given
time point and a signal value is called a transaction. The necessity
of drivers is explained by the presence of unit-delay signal
assignments. A unit-delay signal assignment specify a time clause
indicating when a giving assignment must be performed,
...
effect is to change the driver of the target signal by posting a
transaction between the current time of simulation plus the amount of
time specified in the time clause to the current value of the
expression at the right side of the arrow. Note that without
unit-delay signal assignments, drivers are not needed as all
assignments take their effects at the current simulation time and thus
directly affect the current value of a given signal.
...
performing static type-checking on the concurrent statements
(processes and design instantiation statements) and the sequential
statements (the body of processes). It also checks that the connection
between signals respect certain rules, for instance, that there are no
multiply-driven signals, i.e. signals that are written to by multiple
processes. Finally, the elaboration operates some transformations over
the VHDL code, and thus builds the \emph{execution} model. All
component instantiation statements are unfolded until the top-level
design behavior is only composed of processes.
...
process that will orchestrate the simulation and most important will
handle the time of the simulation. The sea of processes and the kernel
process follow two activity diagrams represented in Figure and.  At
the beginning of the simulation, all processes are executed exactly
once during an initialization phase. Then, the kernel process takes
control and checks the current value of the signals and the
transaction posted in their drivers. If there exists a transaction in
the driver of a signal with a date corresponding to the current time
point, and if the associated value is different from the current value
of the signal, then the current value is replaced by the value
specified in the transaction and the kernel process registers an event
on the signal. All processes that declared the event-rising signal in
their sensitivity list resume their activities, i.e. they execute the
statement body once more, and the current simulation time does not
evolve. This is called a $\delta$ cycle. Then, if all signal values
are stable at the current time point, then, the kernel process looks
up the drivers for the next time point where the value of a given
signal will change. Then, the kernel process advances the simulation
time to this next time point, and the processes that marked the
event-rising signal in their sensitivity lists resume their activities.
