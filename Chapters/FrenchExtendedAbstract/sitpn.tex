...
associées aux transitions sont des expressions Booléennes sur la
...
qui lui sont associées sont \emph{vraies}. La
Figure~\ref{fig:ipn-french} donne un exemple de RdP interprété.
...
temps a atteint l'intervalle de tir. La Figure~\ref{fig:tpn-french}
donne un exemple de RdP temporel. La valeur courante des compteurs de
temps est représentée entre chevrons en dessous des intervalles
temporels associés. En résumé, une transition d'un SITPN est
franchissable si elle est sensibilisée, si toutes les conditions qui
lui sont associées sont vraies et si son compteur de temps est dans
l'intervalle défini.
...
Figure~\ref{fig:sync-exec-french}. Sur le front descendant (\textcircled{1}
de la Figure~\ref{fig:sync-exec-french}), toutes les transitions devant être
...
Figure~\ref{fig:sync-exec-french}), les précédentes transitions sont tirées,
...
exemple, sur le front descendant d'horloge, la liste des transitions à
...
nous avons utilisé une autre version de l'implantation des SITPNs en
\coq{} pour effectuer la preuve de préservation sémantique. La
