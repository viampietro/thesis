    ...
    \item \textbf{CASE} $input(t)\neq\emptyset$:\\
      ...
      \item \textbf{CASE} $pre(p,t)=(\omega,\mathtt{BASIC})$:\\
        ...
        \qedbox{$s'.M(p)\ge\omega=\mathtt{true}$.}\footnote{Here $\ge$
          ...
          the $\ge\subseteq{}(\mathbb{N}\times\mathbb{B})$ relation is
...
\begin{proof}
  For all $sitpn\in{}SITPN$, $s\in{}S(sitpn)$ and $t\in{}T$,
  $t\in{}Sens(s'.M)$ is decidable. Therefore, proving the above lemma
  is trivial by appealing to Lemma~\nameref{lem:re-equal-sens} and by
  reasoning on contrapositives.
