...
In the same manner as the bisimulation diagrams of
Figure~\ref{fig:sim-diagrams}, Figure~\ref{fig:bisim-diagram} gives
the kind of bisimulation diagram followed by the execution of an SITPN
and its corresponding \hvhdl{} design over one clock cycle. It informs
about the moments during the clock cycle when the SITPN state and the
\hvhdl{} design state are compared, and the kind of similarity that
relates the states at these moments. In
Figure~\ref{fig:bisim-diagram}, the states are compared with the full
state similarity relations. This is because the use of the full state
similarity relations are mandatory to prove Lemmas~\nameref{lem:re}
and \nameref{lem:fe}. However, the \textit{simple} state similarity
relations are sufficient, in relation to our standards, to state the
semantic preservation property. The proof of the upper part of diagram
is given by Lemma~\nameref{lem:fe}, and the proof of the lower part is
given by Lemma~\nameref{lem:re}.
