In this section, we lay out the major theorems and lemmas stating that
the \hilecop{} transformation function is semantic preserving. We also
present the informal proofs for these theorems and lemmas.
...
permits to compare such traces.
...
Definition) SITPN. As a complementary task, we could show that if the
transformation function returns a couple \hvhdl{} design and binder,
and not an error, then the input SITPN is well-defined. To prove
Theorem~\ref{thm:beh-pres}, we must first exhibit an elaborated
version of the returned \hvhdl{} design (Theorem~\ref{thm:elab-ex}),
an initial state (Theorem~\ref{thm:init-ex}), and a simulation trace
over $\tau$ simulation cycles (Theorem~\ref{thm:sim-ex}). Finally, we
can establish that the behaviors are similar by comparing the
respective SITPN execution and \hvhdl{} design simulation traces
...
Theorem~\ref{thm:full-bisim}. For now, we choose to consider
...
\begin{niproof}
  ...
  Appealing to Theorems~\ref{thm:elab-ex}, \ref{thm:init-ex} and
  \ref{thm:sim-ex}, let us take an elaborated design
...
transformation function; that is, if the input SITPN model is
...
\begin{remark}[Bounded SITPN and behavior preservation]
  ...
  \texttt{s\_marking} signals in the internal states of PCIs.  In that
  case, there exist a clock cycle count where the next design state
  can never be derived because of the overflow. Passed this clock
  cycle count, the simulation of the \hvhdl{} design ends, the
...
Theorems~\ref{thm:elab-ex}, \ref{thm:init-ex} and \ref{thm:sim-ex}
will be tackle down in future works. We chose to focus our work on the
...
Lemma~\ref{lem:sim-init-states}.  The first execution cycle is
...
appealing to Lemma~\ref{lem:fst-re}. The detailled proofs for
Lemmas~\ref{lem:sim-init-states} and \ref{lem:fst-re} are
...
\begin{niproof}
  ...
  \framebox{$\gamma\vdash\theta_s\sim\theta_\sigma$}.
  ...
  \item \textbf{CASE} $\tau=0$. By definition of the SITPN full
    ...
    Appealing to Lemma~\ref{lem:sim-init-states}, we can show
  ...
  \item \textbf{CASE} $\tau>0$. By definition of the SITPN full
    ...
    Rewriting $\theta_s$ and $\theta_\sigma$, the new goal is:
    \framebox{$\gamma\vdash{}(s_0 :: s_0 :: s ::
      \theta)\sim{}(\sigma_0 :: \theta')$}
    ...
    Rewriting $\theta'$, the new goal is:
    ...
    \item
      ...
      Appealing to Lemma~\ref{lem:fst-re}, we have
...
\begin{lemma}[Bisimulation]
  ...
  $E_p\in\mathbb{N}\rightarrow{}Ins(\Delta)\rightarrow{}value$,
  $E_c\mathbb{N}\rightarrow\mathcal{C}\rightarrow\mathbb{B}$,
...
\begin{niproof}
  ...
  \item \textbf{INDUCTION CASE}: Assuming the following induction
    ...
    \item
      ...
      Appealing to Lemma~\ref{lem:fe}, we have
    ...
    \item
      ...
      Appealing to Lemmas~\ref{lem:fe} and \ref{lem:re}, we
      can show
...
The upper part of the diagram is proved by Lemma~\ref{lem:re}. First,
we assume that the starting SITPN state and the starting \hvhdl{}
design state verify the full post falling edge state similarity
relation at the beginning of the clock cycle (i.e,
...
proved by Lemma~\ref{lem:fe}. First, we assume that the starting SITPN
state and the starting \hvhdl{} state verify the full post rising edge
state similarity relation (i.e,
$s'\stackrel{\downarrow}{\approx}\sigma'$ in
Figure~\ref{fig:bisim-diagram}). Then, Lemma~\ref{lem:re} states that
...
Here, we present Lemma~\ref{lem:re} and Lemma~\ref{lem:fe}, along with
their proofs. In the two lemmas, we added an extra hypothesis about
the starting state of the \hvhdl{} design:
...
To prove Lemmas~\ref{lem:re} and \ref{lem:fe}, one must show that
...
to show the equalities or equivalences laid out in the state
...
\begin{niproof}
  ...
  \begin{frameb}
    ...
    \item
      ...
      $\sigma'(id_t)(\texttt{s\_time\_counter})=lower(I_s(t))\big)$\\
      $\land\big(upper(I_s(t))\neq\infty\land{}s'.I(t)>{}upper(I_s(t))\Rightarrow$
      $\sigma'(id_t)(\texttt{s\_time\_counter})=upper(I_s(t))\big)$\\
  ...
  \begin{itemize}[label=--]
  \item Apply the \nameref{lem:re-equal-marking} lemma to solve
    \ref{it:marking-eq-re}.
  \item Apply the \nameref{lem:re-equal-tc} lemma to solve
    \ref{it:time-count-eq-re}.
  \item Apply the \nameref{lem:re-equal-reset-orders} lemma to solve
    \ref{it:reset-eq-re}.
  \item Apply the \nameref{lem:re-equal-action-exec} lemma to solve \ref{it:action-eq-re}.
  \item Apply the \nameref{lem:re-equal-fun-exec} lemma to solve
    \ref{it:fun-eq-re}.
  \item Apply the \nameref{lem:re-equal-sens} lemma to solve
    \ref{it:sens-eq-re}.
  \item Apply the \nameref{lem:re-equal-not-sens} lemma to solve
    \ref{it:not-sens-eq-re}.
  \item Apply the \nameref{lem:re-equal-cond-comb} lemma to solve
    \ref{it:cond-comb-eq-re}.
  \item Apply the \nameref{lem:re-equal-cond} lemma to solve
    \ref{it:cond-eq-re}.
  \end{itemize}
\end{niproof}
...
\begin{niproof}
  ...
  \begin{frameb}
    ...
    \item
      $\forall{}t\in{}T_i,id_t\in{}Comps(\Delta)~s.t.~\gamma(t)=id_t,$\\
      $\big(upper(I_s(t))=\infty\land{}s'.I(t)\le{}lower(I_s(t))\Rightarrow{}s'.I(t)=\sigma'(id_t)(\texttt{s\_time\_counter})\big)$\\
      $\land\big(upper(I_s(t))=\infty\land{}s'.I(t)>{}lower(I_s(t))\Rightarrow{}\sigma'(id_t)(\texttt{s\_time\_counter})=lower(I_s(t))\big)$\\
      $\land\big(upper(I_s(t))\neq\infty\land{}s'.I(t)>{}upper(I_s(t))\Rightarrow{}\sigma'(id_t)(\texttt{s\_time\_counter})=upper(I_s(t))\big)$\\
      $\land\big(upper(I_s(t))\neq\infty\land{}s'.I(t)\le{}upper(I_s(t))\Rightarrow{}s'.I(t)=\sigma'(id_t)(\texttt{s\_time\_counter})\big)$.
  ...
  Each point is proved by a separate lemma:
  \begin{itemize}[label=--]
  \item Apply the \nameref{lem:fe-equal-marking} lemma to solve
    \ref{item:fe-equal-marking}.
  \item Apply the \nameref{lem:fe-equal-tc} lemma to solve
    \ref{item:fe-equal-tc}.
  \item Apply the \nameref{lem:fe-equal-cond-values} lemma to solve
    \ref{item:fe-equal-cond-values}.
  \item Apply the \nameref{lem:fe-equal-act-exec} lemma to solve
    \ref{item:fe-equal-act-exec}.
  \item Apply the \nameref{lem:fe-equal-fun-exec} lemma to solve
    \ref{item:fe-equal-fun-exec}.
  \item Apply the \nameref{lem:fe-equal-firable} lemma to solve
    \ref{item:fe-equal-firable}.
  \item Apply the \nameref{lem:fe-equal-not-firable} lemma to solve
    \ref{item:fe-equal-not-firable}.
  \item Apply the \nameref{lem:fe-equal-fired} lemma to solve
    \ref{item:fe-equal-fired}.
  \item Apply the \nameref{lem:fe-equal-not-fired} lemma to solve
    \ref{item:fe-equal-not-fired}.
  \item Apply the \nameref{lem:fe-equal-ots} lemma to solve
    \ref{item:fe-equal-ots}.
  \item Apply the \nameref{lem:fe-equal-its} lemma to solve
