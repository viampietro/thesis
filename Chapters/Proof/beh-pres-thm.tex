...
correspondence between constants and signals, and their aliases. Also,
during a proof and when there is no ambiguity, $id_p$ (resp. $id_t$)
denotes the PCI (resp. TCI) identifier associated to a given place $p$
...
SITPN execution and \hvhdl{} simulation traces
...
\begin{niproof}
  ...
  $\Delta\in{}ElDesign(d,\mathcal{D}_\mathcal{H})$, two design states
  $\sigma_e,\sigma_0\in\Sigma(\Delta)$, and a simulation trace
  $\theta_\sigma\in{}$ such that:
  \begin{itemize}
...
code, i.e. code that could be the input to a simulator program.
...
\begin{thm}[Simulation]
  \label{thm:sim-ex}
  For all well-defined $sitpn\in{}SITPN$, $d\in{}design$, $\gamma\in{}WM(sitpn,d)$,
  $\Delta\in{}ElDesign(d,\mathcal{D}_\mathcal{H})$,
  ...
  $E_p\in{}(\mathbb{N}\times{}\{\uparrow,\downarrow\})\rightarrow{}Ins(\Delta)\rightarrow{}value$,
  and simulation cycle count $\tau\in\mathbb{N}$, there exists a
...
\begin{niproof}
  ...
  \begin{frameb}
    ...
    \item
      $\forall{}p\in{}P,id_p\in{}Comps(\Delta)~s.t.~\gamma(p)=id_p,$
      $\sum\limits_{t\in{}Fired(s')}pre(p,t)=$ $\sigma'(id_p)("s\_output\_token\_$ $sum")$.\label{item:fe-equal-ots}
    \item
      $\forall{}p\in{}P,id_p\in{}Comps(\Delta)~s.t.~\gamma(p)=id_p,$
