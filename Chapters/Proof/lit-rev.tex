...
the proof of semantic preservation theorems, with an extra interest
...
shows the different kind of simulation diagrams possibly relating two
programs.
...
dealing with programming languages). The memory model consists in
block references; each block has a lower and an upper bound. To access
a data, one has to specify the block reference along with the size of
...
statements, and finally for an entire Clight program. The proof
...
state also hold a log of the read and write operations of the input
...
the semantics define a function $step$ that computes the execution the
...
semantics by means of an interpreter that runs a netlist list over n
