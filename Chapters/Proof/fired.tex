...
establish the semantic preservation property. We use the proof of the
Lemma~\ref{lem:fe-equal-fired}, involved in the proof of
...
at the beginning of the rising edge phase, i.e. when the set of fired
transitions will be used to compute a new SITPN state. To express
...
\begin{figure}[H]
  ...
  \caption[An set of fired transitions]{The set of fired
...
declares an output port of Boolean type with the identifier
...
signals are stable, i.e. at the end of the stabilization phases
happening during the simulation. In the point of view of the circuit
synthesis, this equation reflects the wiring of the port in the
described hardware circuit. Figure~\ref{fig:fired-port} shows a part
of the transition design architecture describing how the
\texttt{fired} port is connected to the other internal signals.
...
equation associated to the \texttt{fired} port. Given a TCI $id_t$ in
a top-level design and a state $\sigma$ denoting a current stable
state of the design (remember that combinational equation hold when
the signal values are stable), the \texttt{fired} port equation at
$\sigma$ is:
...
\end{lstlisting}
  ...
  In Equation~\eqref{eq:spc-comb-eq}, ``spc'' is an alias for the
...
these two terms in the current goal; what is left to prove is:
...
of the \texttt{priority\_authorizations} input port of the transition
...
\begin{figure}[H]
  ...
  ports between a PCI and a TCI.]{Connection of the $j$-th element of
    the \texttt{priority\_authorizations} output port of $id_p$ to the
    $i$-th element of the \texttt{priority\_authorizations} input port
    of $id_t$; also the \texttt{fired} output port of $id_t$ is
    connected to the $j$-th element of the
    \texttt{output\_transitions\_fired} input port of $id_p$.}
...
output port of $id_p$. Thus, to show that
\fbox{$\sigma'(id_t)("pauths")[i]=\mathtt{true}$}, we must now show
that \fbox{$\sigma'(id_p)("pauths")[j]=\mathtt{true}$.} We must now
look at the architecture of the place design to determine the
combinational equation associated to the $j$-th element of the
...
8 assigns the result of the test \vhdle|s_marking $-$ v_saved_output_token_sum >= output_arcs_weights(k)| to the $k$-th
...
through an index $l$ comprised between 0 and $j-1$. The output arc
...
equals \texttt{true} at index $l$) and is linked to the place by a
\texttt{basic} input arc (i.e. the \texttt{output\_arcs\_types} input
port of $id_p$ equals \texttt{basic} at index $l$). The order of the
indexes from 0 to $\mathtt{output\_arcs\_number}-1$ reflects the
priority order of the output transitions of place $p$. Therefore, the
indexes from 0 to $j-1$ are linked to transitions with a higher firing
priority than the transition connected to the index
$j$. Figure~\ref{fig:ordered-indexes} reuses the SITPN of
...
iterates until all transitions of the $T_s$ set have been elected for
firing or have been discarded. At Line 4, function
\texttt{GetTopPriorityTransitions} returns the $tp$ set of
...
top-priority transitions in $T_s$. Thus, $tp$ is never empty and $T_s$
is always decrementing after the assignment of Line 7. Thus, the
algorithm always terminates and returns the set of fired transitions
at state $s$.
...
set of fired transitions is no longer involved in any the SITPN
semantics rules happening during the falling edge phase. Therefore, we
chose to withdraw the $Fired$ set from the definition of the SITPN
state record. We opted for an intentional definition of the set of
fired transitions at given SITPN state (i.e.,
