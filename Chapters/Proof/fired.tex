...
\begin{definition}[Falling edge hypotheses]
  \label{def:fe-hyps}
  Given a $sitpn\in{}SITPN$, $d\in{}design$, $\gamma\in{}WM(sitpn,d)$,
...
the priority relation. In that case, the $i$-th of the
\texttt{priority\_authorizations} input port of the transition
component instance $id_t$ is connected to the $j$-th element of the
\texttt{priority\_authorizations} output port of the place component
instance $id_p$. Here, $id_p$ is the place component instance bound to
place $p$ through the $\gamma$ binder, i.e. we have
$\gamma(p)=id_p$. Figure~\ref{fig:pauths-connection} shows the
connection of the \texttt{priority\_authorizations} port between the
component instances $id_p$ and $id_t$.

\begin{figure}[H]
  \centering
  \includegraphics[keepaspectratio, width=.8\textwidth]{Figures/Proof/pauths-connection}
  \caption[Connection of the \texttt{priority\_authorizations} ports and of the \texttt{fired} and \texttt{output\_transitions\_fired} ports.]{Connection of the $j$-th element of the \texttt{priority\_authorizations} output port of a place component instance $id_p$ to the $i$-th element of the \texttt{priority\_authorizations} input port of a transition component instance $id_t$; also the \texttt{fired} output port of $id_t$ is connected to the $j$-th element of the \texttt{output\_transitions\_fired} input port of $id_p$.}
...
output port of $id_p$. Therefore, we must now show that
\fbox{$\sigma'(id_p)("pauths")[j]=\mathtt{true}$.} We must now look at
the architecture of the place design to determine the combinational
equation associated to the $j$-th element of the
\texttt{priority\_authorizations} output
port. Figure~\ref{fig:pauths-in-p} illustrates the wiring of the
\texttt{priority\_authorizations} output port in a place design.

\begin{figure}[H]
  \centering
  \includegraphics[keepaspectratio,width=.7\textwidth]{Figures/Proof/pauths}
  \caption[Wiring of the \texttt{priority\_authorizations} output port
...
\texttt{marking}. It also reads the value of the input ports
\texttt{output\_transitions\_fired}, \texttt{output\_arcs\_types} and
\texttt{output\_arcs\_weights}.  In Figure~\ref{fig:pauths-in-p}, the
ports of the input and output interface are composite ports (i.e., of
the array type) with an upper bound index equal to \texttt{m}. The
number $\mathtt{m}$ is equal to the expression
$\mathtt{output\_arcs\_number}-1$, where
$\mathtt{output\_arcs\_number}$ is a generic constant of the place
design. The value of the $\mathtt{output\_arcs\_number}$ constant is
set at the generation of the generic map of a place component instance
$id_p$, and is equal to the number of output transitions of place
$p$. Listing~\ref{lst:pauths-out-eval-ps} presents the code of the
\texttt{priority\_evaluation} process.

\begin{lstlisting}[language=VHDL,numbers=left,label={lst:pauths-out-eval-ps},caption={The \texttt{priority\_evaluation} process in the place design's architecture.}]
...
each element of the \texttt{priority\_authorizations} output port is
assigned at Line 8 inside the \texttt{for} loop. The statement of Line
8 assigns the result of the test \vhdle|s_marking $-$ v_saved_output_token_sum >= output_arcs_weights(k)|
to the $k$-th element of \vhdle|priority_authorizations|.  The test checks that
the value of the \texttt{s\_marking} signal, representing the current marking of the PCI, minus
the value of the local variable \texttt{v\_saved\_output\_token} is greater than or equal to the value of the $k$-th element
of the \texttt{output\_arcs\_weights} signal. The test corresponds to the test of sensitization by
the residual marking for the transition component instance connected through index $k$.

Getting back to our proof, the following combinational equation holds
the $j$-th element of the \texttt{priority\_authorizations} port at
state $\sigma'$:
\begin{equation}
  \label{eq:pauths-j-comb-eq}
  \sigma'(id_p)("pauths")[j]=(\sigma'(id_p)("s\_marking")-\mathtt{vsots}\ge\sigma'(id)("output\_arcs\_weights")[j])
\end{equation}

\noindent{}Then, rewriting the goal with Equation~\eqref{eq:pauths-j-comb-eq},
the new goal is:\\
\fbox{$(\sigma'(id_p)("s\_marking")-\mathtt{vsots}\ge\sigma'(id)("output\_arcs\_weights")[j])=\mathtt{true}$.}
...
\fbox{$\sigma'(id_p)("s\_marking")-\mathtt{vsots}\ge\sigma'(id)("output\_arcs\_weights")[j]$.}
...
the value of the \texttt{vsots}:
\begin{equation}
  \label{eq:vsots}
  \mathtt{vsots}=\sum\limits_{l=0}^{j-1}
  \begin{cases}
    \sigma'(id_p)("oaw")[l]~\mathtt{if}~\sigma'(id_p)("otf")[l].\\
    \hspace{19ex}\sigma'(id_p)("oat")[l]=\mathtt{basic}\\
    0~otherwise\\
  \end{cases}
\end{equation}

The \texttt{vsots} term is equal to the sum of the output arc weights
for all TCIs with an index comprised between 0 and $j-1$ that are
\texttt{fired} and linked to the place by a \texttt{basic} input
arc. The order of the indexes from 0 to
$\mathtt{output\_arcs\_number}-1$ reflects the priority order of the
output transitions. Therefore, the indexes from 0 to $j-1$ are linked
to transitions with a higher firing priority than the transition
connected to the index $j$. Figure~\ref{fig:ordered-indexes} takes
back the SITPN of Figure~\ref{fig:example-fired-trans} and illustrates
how the indexes are ordered when the connection between the PCI $id_p$
and its output TCIs $id_{t_a}$, $id_{t_b}$ and $id_{t_c}$ is set (i.e.,
...
top-priority transition in the output transitions of $p$.

As a reminder, the current goal to prove is:\\
\fbox{$\sigma'(id_p)("s\_marking")-\mathtt{vsots}\ge\sigma'(id)("output\_arcs\_weights")[j]$.}

The current goal is the \hvhdl{} implementation of the test that the
residual marking in place $p$ enables transition $t$. We made the
hypothesis that transition $t$ is sensitized by the residual marking
for all its input places, i.e.
$t\in{}Sens(s'.M-\sum\limits_{t_i\in{}Pr(t,s')}pre(t_i))$. By looking
at the definition of $Sens$, and knowing that a basic arc of weight
$\omega$ connects place $p$ to transition $t$, we can deduce that
$s'.M(p)-\sum\limits_{t_i\in{}Pr(t,s')}pre(p,t_i)\ge\omega$. Now, we
must relate the terms of the preceding formula to the terms of the
goal. We can easily show, appealing to
Lemma~\ref{lem:fe-equal-marking}, that $s'.M(p)$ equals
$\sigma'(id_p)("s\_marking")$. Then, by construction, and knowing that
TCI $id_t$ is connected to PCI $id_p$ through the index $j$, we can
deduce that the $j$-th element of the \texttt{output\_arcs\_weights}
input port denotes the weight of the arc between place $p$ and
transition $t$, i.e. $\omega$. The last thing to show is the equality
between the two sum terms:\\
\noindent{}\fbox{$\sum\limits_{t_i\in{}Pr(t,s')}
\begin{cases}
...
On the left side of the equality, we have unfolded term
$\sum\limits_{t_i\in{}Pr(t,s')}pre(t_i)$ to its full definition (see
Remark in Section\todo{add ref}). On the right side is the full
definition of term \texttt{vsots}. We can reason by induction over the
sum terms of the goal to complete the proof. At some point, we will
have to show that there is a relation between an index $l\in{}[0,j-1]$
and a transition $t_i\in{}Pr(t,s')$. Thanks to the ordering of the
indexes based on the priority order of output transitions (see
Figure~\ref{fig:ordered-indexes}), we know that there is a bijection
between the output transitions of $p$ with a higher priority than $t$
and the indexes of interval $[0,j-1]$. However, to complete the proof,
we must assume that for a given transition $t_i$ with a higher
priority than $t$ and its corresponding TCI $id_{t_i}$ the ``fired''
equivalence holds, i.e.:
$t_i\in{}Fired(s')\Leftrightarrow\sigma'(id_{t_i})("fired")=\mathtt{true}$. Unfortunately,
this is exactly the property we are currently trying to prove.
...
priority than $t$. Therefore, we must think of a way to build the set
of fired transitions iteratively such that the previous hypothesis
becomes an invariant over the many iterations.  As stated before, the
actual definition of the set of fired transitions is very declarative.
However, we can easily convert it into an algorithm that will build
the set iteratively. The result is Algorithm~\ref{alg:fired}.
...
\begin{algorithm}[H]
  ...
  \While{$T_s\neq\emptyset$}{
    $tp\leftarrow$ \texttt{GetTopPriorityTransitions($T_s$, $\succ$)}\;
    $f\leftarrow$ \texttt{ElectFired($s$,$tp$,$F$)} % \tcp*{$f\subseteq{}tp$}
    \;
    $F\leftarrow{}F\cup{}f$\;
    $T_s\leftarrow{}T_s\setminus{}tp$ % \tcp*{$T_s$ is always decrementing because $\succ$ is a strict order.}
    \;
  }
...
the beginning of the algorithm. At Line 3, the \texttt{while} loop
iterates until all transitions of the set $T_s$ have been elected for
firing or have been discarded. At Line 4, function
\texttt{GetTopPriorityTransitions} returns the set $tp$ of
top-priority transitions inside $T_s$.  Then, we can proceed to the
election of the fired transitions inside $tp$. We know that the
following loop invariant holds: all fired transitions with a higher
firing priority than the transitions of the $tp$ set are inside set
$F$. Therefore, set $F$ contains all the transitions necessary to
compute the residual marking that will be used to elect the fired
transitions inside $tp$. This is done by the \texttt{ElectFired}
function at Line 5. Then, the set $f$ of fired transitions inside $tp$
is merged with the overall set of fired transitions $F$. The statement
of Line 7 withdraws the transitions of $tp$ from set $T_s$ before
beginning another iteration. Because the priority relation $\succ$ is
a strict order over the set of transitions $T$, we can always find
top-priority transitions in $T_s\subseteq{}T$. Thus, $tp$ is never
empty and $T_s$ is always decrementing after the assignment of Line
7. Thus, the algorithm always terminates and returns the set of fired
transitions at state $s$.

We make a relational definition of Algorithm~\ref{alg:fired} through
the definition of the $IsFiredSet$ relation given in
Definition~\ref{def:is-frd-set}. Definition~\ref{def:cons-fired}
states that a given transition is fired in relation to the
$IsFiredSet$ relation.
...
transitions provided through the $IsFiredSet$ relation. Therefore, we
give a new expression to Lemma~\ref{lem:fe-equal-fired} by using the
$IsFiredSet$ relation to qualify the set of fired transitions instead
of using the first declarative definition. The result is
Lemma~\ref{lem:fe-equal-fset}. The full formal proof of
Lemma~\ref{lem:fe-equal-fset} is given in
Section~\ref{sec:falling-edge} of
Appendix~\ref{app:sem-preserv-proof}.

The definition of the $IsFiredSet$ relation depends on the definition
of the $IsFiredSetAux$ relation given in
Definition~\ref{def:is-frd-set-aux}. The inductive definition of the
...
\begin{definition}[IsFiredSetAux]
  ...
  \begin{tabular}{@{}l}
    ...
    {\begin{prooftree}[template={\inserttext}]        
        \infer0{$IsFiredSetAux(s, fired, \emptyset, fired)$}
      \end{prooftree}} 
  \end{tabular}
  \begin{tabular}{@{}l}
    ...
    {\begin{prooftree}[template={\inserttext}]
        ...
        \hypo{$IsTopPrioritySet(T_s, tp)$}
        \infer[no rule]1{$ElectFired(s, fired, tp, fired')$}
        \infer[no rule]1{$IsFiredSetAux(s, fired', T_s\setminus{}tp, fset)$}
        \infer1{$IsFiredSetAux(s, fired, T_s, fset)$}
      \end{prooftree}} 
  \end{tabular}
\end{definition}

The $IsFiredSetAux$ relation depends on the definitions of the
$IsTopPrioritySet$ and the $ElectFired$ relations given in
Definition~\ref{def:is-tp-set} and \ref{def:is-tp-set}. The
$IsTopPrioritySet$ is a relational implementation of the
\texttt{GetTopPriorityTransitions} function appearing at Line 4 of
Algorithm~\ref{alg:fired}. The $ElectFired$ relation is a relational
implementation of the \texttt{ElectFired} function appearing at Line 5
of Algorithm~\ref{alg:fired}.

\begin{definition}[IsTopPrioritySet]
  \label{def:is-tp-set}
  Given an $sitpn\in{}SITPN$, a
  subset $T_s\subseteq{}T$, and a subset $tp\subseteq{}T$, the $IsTopPrioritySet$ relation is defined as follows:\\
  $IsTopPrioritySet(T_s,tp)\equiv{}IsTopPrioritySetAux(T_s,\emptyset,\emptyset,tp)$
\end{definition}

\begin{definition}[IsTopPrioritySetAux]
  \label{def:is-tp-set-aux}
  The $IsTopPrioritySetAux$ relation is defined by the following
  rules:
  
  \begin{tabular}{@{}l}
    {\fontsize{9}{11}\selectfont\textsc{IsTPSetAuxEmpty}} \\
    
    {\begin{prooftree}[template={\inserttext}]
        \infer0{$IsTopPrioritySetAux(\emptyset, T_b, tp, tp)$}
      \end{prooftree}} 
  \end{tabular}
  \begin{tabular}{@{}l}
    {\fontsize{9}{11}\selectfont\textsc{IsTPSetAuxTP}} \\
    
    {\begin{prooftree}[template={\inserttext}]
        \hypo{$\forall{}t'\in{}T_a\cup{}T_b,t'\nsucc{}t$}
        \infer[no rule]1{$IsTopPrioritySetAux(T_a, \{t\}\cup{}T_b, \{t\}\cup{}tp, tp')$}
        \infer1{$IsTopPrioritySetAux(\{t\}\cup{}T_a, T_b, tp, tp')$}
      \end{prooftree}} 
  \end{tabular}

  \begin{tabular}{@{}l}
    {\fontsize{9}{11}\selectfont\textsc{IsTPSetAuxNTP}} \\
    
    {\begin{prooftree}[template={\inserttext}]
        \hypo{$\exists{}t'\in{}T_a\cup{}T_b~s.t.~t'\succ{}t$}
        \infer[no rule]1{$IsTopPrioritySetAux(T_a, \{t\}\cup{}T_b, tp, tp')$}
        \infer1{$IsTopPrioritySetAux(\{t\}\cup{}T_a, T_b, tp, tp')$}
      \end{prooftree}} 
  \end{tabular}
\end{definition}
...
\begin{definition}[ElectFired]
  \label{def:elect-fired}
  The $ElectFired$ relation is defined by the following rules:
  
  \begin{tabular}{@{}l}
    {\fontsize{8}{11}\selectfont\textsc{ElectFiredEmpty}} \\
  ...
  \begin{tabular}{@{}l}
    ...
    {\begin{prooftree}[template={\inserttext},right label template={\fontsize{9}{11}\selectfont\inserttext}]
        \hypo{$\neg\big(t\in{}Firable(s)\wedge{}t\in{}Sens(s.M-\sum\limits_{t_i\in{}Pr(t,fired)}pre(t_i))\big)$}
        \infer[no rule]1{$ElectFired(s, fired, tp, fired')$}
        \infer1[$Pr(t,fired)=\{t'~\vert~t'\succ{}t\land{}t'\in{}fired\}$]{$ElectFired(s, fired, \{t\}\cup{}tp, fired')$}
      \end{prooftree}} 
  \end{tabular}
  ...
  \begin{tabular}{@{}l}
    ...
    {\begin{prooftree}[template=\inserttext,right label template={\fontsize{9}{11}\selectfont\inserttext}]
        \hypo{$t\in{}Firable(s)$}
        \hypo{$t\in{}Sens(s.M-\sum\limits_{t_i\in{}Pr(t,fired)}pre(t_i))$}
...
effective at the end of the rising edge phase. In the previous SITPN
semantics, the set of fired transitions stayed the same during the
rising edge phase. Therefore, between two SITPN states $s, s'$
...
rising edge phase. The consequence was a divergence between the value
of time counters and the value of the \texttt{s\_time\_counter}
signals. Figure shows a case of divergence between time counters and
\texttt{s\_time\_counter} signals values in the course of an
execution.

In Figure, during the stabilization phase coming right after the
rising edge of the clock, the value of the \texttt{fired} port of TCI
$id_{t_1}$ passes to \texttt{false}. After the update of the
\texttt{s\_marking} signal value during the rising edge phase, PCI
$id_p$ computes new priority authorizations for its output TCIs. As
the marking is only sufficient to fire transition $t_0$ but not
transition $t_1$, PCI $id_p$ indicates to TCI $id_{t_1}$ that it no
longer has the authorization to fire. Consequently, through the
connection of \texttt{priority\_authorizations} ports, the value of
the \texttt{fired} port of $id_{t_1}$ is set to \texttt{false}.
...
the moment where the equivalence hold (i.e. at the end of the falling
...
the \texttt{s\_reinit\_time\_counter} signal. Thus, the
\texttt{time\_counter} process can rely on the value of the
\texttt{s\_reinit\_time\_counter} signal to reset the value of the
\texttt{s\_time\_counter} signal. As a consequence, the set of fired
transitions is no longer involved in any the SITPN semantics rules
happening during the falling edge phase. Therefore, we chose to
withdraw the $Fired$ set from the definition of the SITPN state
record. We opted for an intentional definition of the set of fired
transitions at given SITPN state (i.e.,
Definition~\ref{def:fired}). After these changes, we were able to
prove that there were no more divergence between the time counters and
the value of the \texttt{s\_time\_counter} signals in the course of
the execution (see Lemmas~\ref{lem:re-equal-tc} and
\ref{lem:fe-equal-tc} about the equivalence of time counters).
