...
looking at the \vhdl{} code, we are able to determine the
combinational equation associated to the \texttt{fired} port. Given a
TCI $id_t$ in a top-level design and a state $\sigma$ denoting a
current stable state of the design (remember that a combinational
equation holds when all signal values are stable), the \texttt{fired}
port equation at $\sigma$ is:
