...
\begin{definition}[Falling Edge Hypotheses]
  \label{def:fe-hyps}
  Given an $sitpn$, $d$, $\gamma$, $\Delta$, $\sigma_e$, $E_c$, $E_p$,
  $\tau$, $s$, $s'$, $\sigma$, $\sigma_i$, $\sigma_{\downarrow}$, $\theta$,
  $\sigma'$, assume that:
  \begin{itemize}
  ...
  \item $E_c,\tau\vdash{}sitpn,s\srarrow{\downarrow}{\fontsize{5}{7}\selectfont}s'$
  \item $\mathtt{Inject}_\downarrow(\sigma, E_p, \tau, \sigma_i)$
    ...
    and
    $\Delta,\sigma_{\downarrow}\vdash\mathrm{d.cs}\xrightarrow{\theta}\sigma'$
  \end{itemize}
\end{definition}

\def\fehyps{For all $sitpn$, $d$, $\gamma$, $\Delta$, $\sigma_e$,
  $E_c$, $E_p$, $\tau$, $s$, $s'$, $\sigma$, $\sigma_i$,
  $\sigma_{\downarrow}$, $\theta$, $\sigma'$ that verify the hypotheses of
  Def.~\ref{def:fe-hyps},}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%% FALLING EDGE PREPARE MARKING UPDATE LEMMA %%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\begin{lemma}[Falling Edge Prepare Marking Update]
  \label{lem:fe-prepare-marking}
  \fehyps{} then $\forall{}p,id_p~s.t.~\gamma(p)=id_p$ and
  $\sigma'(id_p)=\sigma'_p$:
  \begin{itemize}
  \item
    $~\sum\limits_{t\in{}Fired(s')}pre(p,t)=\sigma'_p("s\_output\_token\_sum")$
  \item   
...
\begin{proof}
  First, by reasoning on the VHDL falling and stabilize relation, and
  on the VHDL Place component behavior, we can unfold the value of
  signals ``s\_input\_token\_sum'' and ``s\_output\_token\_sum'' at
  state $\sigma'_p$.
  \begin{itemize}

  \item $\sigma'_p("s\_input\_token\_sum")=\sum\limits_{i\in\mathtt{FIIdx}(\sigma'_p)}\sigma'_p("input\_arcs\_weights")(i)$\\
    where $i\in\mathtt{FIIdx}(\sigma)\equiv{}i\in[0,\sigma("input\_arcs\_number")-1]$
    
    \hspace{25ex}$~\land~\sigma("input\_transition\_fired")(i)=\mathtt{true}$.
  \item $\sigma'_p("s\_output\_token\_sum")=\sum\limits_{i\in\mathtt{FOIdx}(\sigma'_p)}\sigma'_p("output\_arcs\_weights")(i)$\\
    where $i\in\mathtt{FOIdx}(\sigma)\equiv{}i\in[0,\sigma("output\_arcs\_number")-1]~$

    \hspace{25ex}$\land~\sigma("output\_transition\_fired")(i)=\mathtt{true}$.
  \end{itemize}
  
\item Then, we need to prove the two following equalities:
  \begin{itemize}
  \item $\sum\limits_{t\in{}Fired(s')}pre(p,t)=\sum\limits_{i\in\mathtt{FOIdx}(\sigma'_p)}\sigma'_p("output\_arcs\_weights")(i)$.
  \item $\sum\limits_{t\in{}Fired(s')}post(t,p)=\sum\limits_{i\in\mathtt{FIIdx}(\sigma'_p)}\sigma'_p("input\_arcs\_weights")(i)$.
  \end{itemize}

  We can deduce that:
  
  \begin{itemize}
  \item $\sum\limits_{t\in{}Fired(s')}pre(p,t)=\sum\limits_{t\in{}FI(s',p)}pre(p,t)$\\
    where
    $t\in{}FI(s',p)\equiv{}t\in{}Fired(s')\land\exists\omega~s.t.~pre(p,t)=(\omega,\mathtt{basic})$.
  \item $\sum\limits_{t\in{}Fired(s')}post(t,p)=\sum\limits_{t\in{}FO(s',p)}post(t,p)$\\
    where
    $t\in{}FO(s',p)\equiv{}t\in{}Fired(s')\land\exists\omega~s.t.~post(t,p)=\omega$.
  \end{itemize}

  \noindent{}Then, we have
  $\vert{}FI(s',p)\vert=\vert\mathtt{FIIdx(\sigma'_p)}\vert$
  and
  $\vert{}FO(s',p)\vert=\vert\mathtt{FOIdx(\sigma'_p)}\vert$.
  
  \noindent{}Then, it easier to show the two equalities by showing
  that the sets $FI(s',p)$ (resp. $FO(s',p)$) and
  $\mathtt{FIIdx}(\sigma'_p)$ (resp. $\mathtt{FOIdx}(\sigma'_p)$) are
  in bijection.

  \noindent{}There are 4 subgoals to prove.

  \begin{enumerate}
  \item
    $\forall{}t\in{}FI(s',p),\exists{}i\in\mathtt{FOIdx}(\sigma'_p)~s.t.~pre(p,t)=\sigma'_p("output\_arcs\_weights")(i)$.

    Given a transition $t\in{}FI(s',p)$, by definition:

    \begin{itemize}
    \item $\exists\omega~s.t.~pre(p,t)=(\omega,\mathtt{basic})$.

      Then, by construction, there exists a Transition component
      $id_t\in{}Comps(\Delta)$ implementing transition $t$, and there
      exists an index
      $j\in[0,\vert{}output(p)\vert-1]$, and a signal $sig\in{}Sigs(\Delta)$ such that\\
      $\mathtt{id_t.fired\Rightarrow{}sig\Rightarrow{}id_p.output\_transitions\_fired(j)}$\\
      and $\mathtt{id_p.output\_arcs\_weights(j)\Rightarrow\omega}$\\
      and
      $\mathtt{id_p.output\_arcs\_number\Rightarrow\vert{}output(p)\vert}$.

      Then, by reasoning on the VHDL stabilize relation, we can deduce
      $j\in[0,\sigma'_p("output\_arcs\_number")-1]$\\
      and $\sigma'_p("output\_arcs\_weights")(j)=\omega$.
      
    \item $t\in{}Fired(s')$.

      Thanks to Lemma~\nameref{lem:fe-equal-fired}, we know that
      $\sigma'_t("fired")=\mathtt{true}$.

      Then, by reasoning on the VHDL stabilize relation, we can deduce
      $\sigma'_p("output\_transitions\_fired")(j)=\sigma'(sig)=\sigma'_t("fired")=\mathtt{true}$.
    \end{itemize}

    Then, choose index $j$ to solve the goal.
  \item
    $\forall{}i\in\mathtt{FOIdx}(\sigma'_p),\exists{}t\in{}FI(s',p)~s.t.~pre(p,t)=\sigma'_p("output\_arcs\_weights")(i)$.
  \item
    $\forall{}t\in{}FO(s',p),\exists{}i\in\mathtt{FIIdx}(\sigma'_p)~s.t.~post(t,p)=\sigma'_p("input\_arcs\_weights")(i)$.
  \item
    $\forall{}i\in\mathtt{FIIdx}(\sigma'_p),\exists{}t\in{}FO(s',p)~s.t.~post(t,p)=\sigma'_p("input\_arcs\_weights")(i)$.
  \end{enumerate}
\end{proof}

\begin{lemma}[Falling Edge Computes Output Token Sum]
  \label{lem:fe-computes-out-tok-sum}
  \fehyps{} then\\
  $\forall{}p\in{}P,id_p\in{}Comps(\Delta)~s.t.~\gamma(p)=id_p,$\\
  $\sigma'(id_p)("s\_output\_token\_sum")=$
  $\sum\limits_{i=0}^n\big(\mathtt{if}~\sigma'(id_p)("out\_t\_fired")[i]~\mathtt{then}~\sigma'(id_p)("out\_arcs\_weights")[i]~\mathtt{else}~0\big)$\\
  where $n=\Delta(id_p)("output\_arcs\_number")-1$
\end{lemma}

\begin{proof}
  Given a $p\in{}P$ and a $id_p\in{}Comps(\Delta)$ s.t. $\gamma(p)=id_p$, let us show that\\
  \fbox{\parbox{\linewidth}{$\sigma'(id_p)("s\_output\_token\_sum")=$
      $\sum\limits_{i=0}^n\big(\mathtt{if}~\sigma'(id_p)("out\_t\_fired")[i]~\mathtt{then}~\sigma'(id_p)("out\_arcs\_weights")[i]~\mathtt{else}~0\big)$
      where $n=\Delta(id_p)("output\_arcs\_number")-1$}}\\\\

  \noindent{}Applying the \nameref{lem:stab-computes-out-tok-sum}
  lemma,

  \noindent{}\qedbox{$\sigma'(id_p)("s\_output\_token\_sum")=$
    $\sum\limits_{i=0}^n\big(\mathtt{if}~\sigma'(id_p)("out\_t\_fired")[i]~\mathtt{then}~\sigma'(id_p)("out\_arcs\_weights")[i]~\mathtt{else}~0\big)$
    where $n=\Delta(id_p)("output\_arcs\_number")-1$}
  
\end{proof}

\begin{lemma}[Stabilize Computes Output Token Sum]
  \label{lem:stab-computes-out-tok-sum}
  For all $sitpn\in{}SITPN$, $d\in{}design$, $\gamma\in{}WM(sitpn,d)$,
  $\Delta\in{}ElDesign(d,\mathcal{D}_\mathcal{H})$,
  $\sigma_e,\sigma,\sigma'\in{}\Sigma(\Delta)$, $\tau\in\mathbb{N}$,
  $\theta\in\mathtt{list}(\Sigma(\Delta))$, assume that:
  \begin{itemize}
  \item $\lfloor{}sitpn\rfloor_\mathcal{H}=(d,\gamma)$ and
    $\mathcal{D}_\mathcal{H},\emptyset\vdash\mathrm{d}\srarrow{elab}{\fontsize{5}{7}\selectfont}\Delta,\sigma_e$
  \item $\Delta,\sigma\vdash\mathrm{d.cs}\xrightarrow{\theta}\sigma'$
  \end{itemize}
  then\\
  $\forall{}p\in{}P,id_p\in{}Comps(\Delta)~s.t.~\gamma(p)=id_p,$\\
  $\sigma'(id_p)("s\_output\_token\_sum")=$
  $\sum\limits_{i=0}^n\big(\mathtt{if}~\sigma'(id_p)("output\_transition\_fired")[i]~\mathtt{then}~$
  
  \hspace{41ex}$\sigma'(id_p)("output\_arcs\_weights")[i]~\mathtt{else}~0\big)$\\
...
\begin{table}[h]
  \begin{tabular}{|c|c|}
    ...
    \hline
    $"s\_output\_token\_sum"$ & $"sots"$ \\
    \hline
    $"output\_transition\_fired"$ & $"otf"$ \\
    \hline
    $"output\_arcs\_weights"$ & $"oaw"$ \\
    \hline
    $"output\_arcs\_number"$ & $"oan"$ \\
    \hline
  \end{tabular}
\end{table}
...
\begin{proof}  
  Given a $p\in{}P$ and a $id_p\in{}Comps(\Delta)$ s.t. $\gamma(p)=id_p$, let us show that\\
  \fbox{\parbox{\linewidth}{$\sigma'(id_p)("sots")=$
      $\sum\limits_{i=0}^n\big(\mathtt{if}~\sigma'(id_p)("otf")[i]~\mathtt{then}~\sigma'(id_p)("oaw")[i]~\mathtt{else}~0\big)$
      where $n=\Delta(id_p)("oan")-1$}}\\
  
  \noindent{}Induction on $\Delta,\sigma\vdash\mathrm{d.cs}\xrightarrow{\theta}\sigma'$.

  \begin{itemize}
  \item \textbf{BASE CASE:}
    \begin{itemize}
    \item $\Delta,\sigma\vdash\mathrm{d.cs}\xrightarrow{[~]}\sigma$
    \item $\mathcal{E}(\sigma)=\emptyset$
    \item $\sigma=\sigma'$
    \end{itemize}
    ...
    \rnote{first pb, what's the value of $\sigma(id_p)("sots")$?}
    \bnote{let's have it as an hypothesis that\\
      \fbox{\parbox{\linewidth}{$\sigma(id_p)("sots")=$
          $\sum\limits_{i=0}^n\big(\mathtt{if}~\sigma(id_p)("otf")[i]~\mathtt{then}~\sigma(id_p)("oaw")[i]~\mathtt{else}~0\big)$
          where $n=\Delta(id_p)("oan")-1$}}}
  ...
  \item \textbf{INDUCTION CASE:}
    \begin{itemize}
    \item $\Delta,\sigma\vdash\mathrm{d.cs}\rightarrow\sigma_1$ and
      $\Delta,\sigma\vdash\mathrm{d.cs}\xrightarrow{\theta}\sigma$
    \item $\mathcal{E}(\sigma)\neq\emptyset$ and
      $\mathcal{E}(\sigma')=\emptyset$
    \end{itemize}

    \rnote{Problem: our hypothesis is taken in the induction process.}
    
    \begin{ih}
      $\Big(\sigma_1(id_p)("sots")=\sum\limits_{i=0}^n\big(\mathtt{if}~\sigma_1(id_p)("otf")[i]~\mathtt{then}~\sigma_1(id_p)("oaw")[i]~\mathtt{else}~0\big)\Big)\Rightarrow$\\
      $\sigma'(id_p)("sots")=\sum\limits_{i=0}^n\big(\mathtt{if}~\sigma'(id_p)("otf")[i]~\mathtt{then}~\sigma'(id_p)("oaw")[i]~\mathtt{else}~0\big)$
    \end{ih}
    ...
    \fbox{\parbox{\linewidth}{$\sigma_1(id_p)("sots")=$
        $\sum\limits_{i=0}^n\big(\mathtt{if}~\sigma_1(id_p)("otf")[i]~\mathtt{then}~\sigma_1(id_p)("oaw")[i]~\mathtt{else}~0\big)$}}\\

    \noindent{}By property of
    $\Delta,\sigma\vdash\mathrm{d.cs}\rightarrow\sigma_1$,\\
    \parbox{\linewidth}{$\sigma_1(id_p)("sots")=$
      $\sum\limits_{i=0}^n\big(\mathtt{if}~\sigma(id_p)("otf")[i]~\mathtt{then}~\sigma(id_p)("oaw")[i]~\mathtt{else}~0\big)$}

    \rnote{We can only prove the goal if we know that
      $\sigma(id_p)("otf")=\sigma_1(id_p)("otf")$ and $\sigma(id_p)("oaw")=\sigma_1(id_p)("oaw")$.}
...
\subsection{Falling Edge and Fired}
\label{sec:fe-fired}
...
\subsection{Falling Edge and Firable}
