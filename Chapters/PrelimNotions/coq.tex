...
$m$ is no longer \coqeb{nat} but \coqeb|{ x : nat | x <= n }|. We have
to extract the first part of the $m$ parameter with the help of the
\coqeb{proj1_sig} function. The first part of an element of the
\coqeb|{ x : nat | x <= n }| type corresponds to the natural number
\coqeb{x} verifying the following property \coqeb{x <= n}, and the
second part corresponds to the proof that \coqeb{x} verifies the
property.
