...
model-to-text transformation, and also to formally verify our proof of
...
\coqeb{evale} relation, i.e. \coqeb{s}. Parameters which are not
varying from one construction case to another can be passed as named
parameters while defining an inductive type. The state \coqeb{s} takes
a string identifier as input and yields an \coqeb{option} to a natural
number.  As so, the \coqeb{option} type permits to represent partial
functions.  The identifiers that belong to the domain of state
\coqeb{s} will be: associated with \coqeb{Some} natural number,
...
set the type of elements passed to the \coqeb{Some} constructor.

\todo[inline]{Maybe present record types?}
...
more elements of other types. Give an example of dependent type, let
us take the definition of polymorphic lists that carry their own
...
constructor of the empty list, is the type of lists of length 0.  The
\coqeb{consn} constructor permits to add a new element at the head of
an existing list to build a new list. Thus, the type of the resulting
list is the type of lists of length $n+1$, where $n$ is the length of
the tail list.
...
\textit{sigma} types to types of elements that meets a given
...
property \coqeb{P}. For instance, if we want to define in \coqe{} the
type of natural numbers that are strictly greater than zero, we can do
it as follows:
...
|. We have to extract the first part of the $m$ parameter with the
help of the \coqeb{proj1_sig} function. The first part of an element
of the \coqeb| $\{$ x : nat | x <= n $\}$ | type corresponds to the
natural number \coqeb{x} verifying the following property
\coqeb{x <= n}, and the second part corresponds to the proof that \coqeb{x}
