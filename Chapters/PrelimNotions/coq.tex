...
\cite{Chlipala2013,Paulin-Mohring2012,Bertot2004}.
...
typed lambda-calculus that includes polymorphism, dependent and
inductive types. Thus, the CIC permits us to define programs and types
similarly; both are \textit{terms} of the language. A program is a
term with a certain type, and a type is also a term with a certain
type. The type of a type is called a \textit{sort}. We can mention
three basic sorts built in the \coq{} proof assistant: the
\texttt{Prop} sort which is the type of logical formulas, the
\texttt{Set} sort which is the type of \textit{small} sets, and the
\texttt{Type} sort which encompasses the \texttt{Prop} and
\texttt{Set} sorts.

The \coq{} proof assistant allows us to express logic formulas and to
...
program. This correspondence is a consequence of the
Brouwer-Heyting-Kolmogorov (BHK) interpretation of the
\textit{intuitionistic} logic \cite{Moschovakis2018}. Intuitionistic
logic is the underlying logic built-in the \coq{} proof
assistant. Intuitionistic logic denies the use of the law of excluded
middle to perform proofs. Thus, intuitionistic logic has a
\textit{constructivist} approach of proofs. In the intuitionistic
setting, one has to provide an explicitly built proof to demonstrate a
theorem; one can not rely on pure proof by contradiction by appealing
to the law of excluded middle. Thus, a proof term of the logical
implication $\mathtt{A}\Rightarrow\mathtt{B}$ is equivalent to an
explicitly built program, or a function, of type \coqeb{A -> B},
i.e. a program that takes an element of type $\mathtt{A}$ and yields
an element of type $\mathtt{B}$. Thus, the type \coqeb{A -> (A -> B) -> A} is a valid encoding of the formula
$\mathtt{A}\Rightarrow(\mathtt{A}\Rightarrow{}\mathtt{B})\Rightarrow\mathtt{B}$.
...
whereas the local environment is empty. The application of the
...
be introduced in the local environment by the \textsc{Lam} rule, i.e.
...
Another way to prove the modus ponens theorem is to directly pass the
proof term with the \coqe|exact| tactic.

\begin{flushleft}
\coqeb{Coq <  exact (fun (x : A) => fun (f : A -> B) => f x).}\\
\coqeb{No more subgoals.}\\
\medskip
\coqeb{Coq <  Qed.}\\
\end{flushleft}

Here the term \coqeb{fun (x : A) => fun (f : A -> B) => f x} represents
the lambda-term
$\lambda{}(x:\mathtt{A}).\lambda{}(f:\mathtt{A}\rightarrow\mathtt{B}).(f~x)$
(i.e. the proof term) in the \coq{} syntax.
...
successor to this natural number. This corresponds to the structural
...
environment accessible by the \coq{} kernel. Also, a corresponding
structural induction principle is generated at the evaluation of an
inductive type definition. For instance, the \coqeb{nat_ind} induction
principle is generated at the evaluation of the \coqeb{nat} type. It
is a proof term of the logical formula denoting the structural
induction principle over the \coqe|nat| type, i.e.:
...
structural induction in a proof involving natural numbers. For
...
subgoals must be proved to complete the proof of the \coqeb{gt_pow2}
theorem. These two subgoals correspond to the proof of $P(0)$ and the
proof that assuming $P(n)$ we can show $P(n+1)$, as agreed with
structural induction. Here, the property $P$ is defined by
...
The \coq{} proof assistant permits us to define proof tactics, or
procedures, in order to automatize some proof tasks. At the top-level
of the \coq{} proof assistant, the \coqeb{Ltac} and \coqeb{Ltac2}
languages are the supports for the definition of this kind of
tactics. These languages allow us to compose sequences of tactics, to
perform pattern matching over the local environment and the current
goal in interactive proof mode, to define loops or recursive tactics,
etc. Even though the \coqeb{Ltac} and \coqeb{Ltac2} languages offer a
lot of possibilities, the user willing to implement complex proof
tactics must turn to the \ocaml{} language which the implementation,
and thus meta-language, of the \coq{} proof assistant. For instance,
the \texttt{lia} tactic is implemented partly with the \texttt{Ltac}
language and as a \ocaml{} program. \\
...
The evaluation relation for arithmetic expressions is defined
similarly:
...
natural number.  As so, the \coqeb{option} type permits the definition
of partial functions.  The identifiers that belong to the domain of
state \coqeb{s} will be associated with \coqeb{Some} natural number,
whereas the unreferenced identifiers will be associated with the
\coqeb{None} value of the \coqeb{option} type. The \coqe{Some} and the
\coqeb{None} constructors are the two constructors of the
\coqeb{option} type which is defined in \coq{} as follows:
...
successor of another element of the \coqeb{nat} type, i.e. with the
...
returned if \coqeb{n} is the successor of a certain \coqeb{m}. In that
...
terminating. For consistency reasons, all \coq{} functions must
terminate and must be total. A user willing to implement a
non-terminating function equivalently define the function as an
inductive type where termination limitations do not apply. For
instance, let us say that we want to implement this following
\textit{ill-formed} version of the \coqe|pow| function:
...
Clearly, this function diverges for all \coqe|n| strictly superior to
0. The \coq{} kernel will not allow such a definition; thus, we can
implement the \coqeb{pow} function as the following relation
\coqe{Pow} $\subseteq{}(\mathbb{N}\times\mathbb{N}\times\mathbb{N})$:
...
The \coqe|Pow| relation takes three parameters of the \coqe|nat| type
and projects a value in the \texttt{Prop} type (meaning that the
\coqe|Pow| relation is a predicate). The third \coqe|nat| parameter
corresponds to the result of the computation of $a^n$ given that the
two first parameters are $a$ and $n$. Determining the result of the
computation of $a^n$ is equivalent to finding a natural number $m$
that verifies that \coqeb|Pow a n m| holds. In intuitionistic logic,
finding a proof of the existence of a $m$ such that \coqeb|Pow a n m|
holds amounts to explicitly building such a $m$.  Here, one can notice
that when the second parameter passed to the \coqeb|Pow| relation is
greater than zero, the formation rules of the \coqe|Pow| relation will
not permit us to find a result for the computation. Thus, a tactic
implementing a proof search for a $m$ such that \coqe|Pow a n m| holds
when $n>0$ will never terminate.
...
introduce here the notion of dependent type and how it is expressed
...
simply appeals to the \coq{} built-in subtraction function, passing a
proof that $m$ is less than or equal to $n$ adds a constraint to the
computation of the subtraction. One can see how dependent types can
...
property. Sigma types are \textit{constructivist} versions, coming
from the intuitionistic logic, of existential logic formulas.  A sigma
type expresses the dependence between a parameter and a proof of a
given property that possibly depends on another parameter. As so,
sigma types are useful to express intentional sets
...
Here, we can no longer directly subtract $n$ with $m$ as the type of
