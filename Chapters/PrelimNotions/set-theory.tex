...
\item a predicate (i.e. an atomic formula). A predicate $P$ possibly
  takes $n$ parameters as inputs. We write $P(x_0,\dots,x_n)$ to
  denote an $n$-ary predicate $P$ applied to the inputs
  $x_0,\dots,x_n$. 
...
\item a subformula prefixed by the universal quantifier $\forall$ or
  ...
  the existantial quantifier $\exists$. Variables that are introduced
  ...
  $\forall{}x.~P(x)\land{}Q(y)$, $x$ is a bound variable and $y$ is a
  free variable of the formula.
\end{itemize}
