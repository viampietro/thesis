...
\item the inadequateness of the \hvhdl{} semantics: during the
  ...
  \texttt{place} and \texttt{transition} design behaviors. Some
  changes have been devised in the \hvhdl{} semantics, and the final
  result has been presented in Chapter~\ref{chap:hvhdl}. However, at
  the moment of the writing, we haven't yet measure the impact of
...
\subsection{Future works and perspectives}
\label{sec:future}
...
\begin{figure}[H]
  ...
  \caption[An SITPN model with a macroplace.]{The macroplace is the
    ...
    the macroplace are particular arcs, and thus with a particular
    semantics, represented with dotted arrows.}
...
transformation function. It also brings a new \hvhdl{} design, the one
for macroplaces, in the loop.
...
\textit{asynchronous} message sending does the bridge between two
parts with different clock domains, that is, when these parts must
communicate together. The system is said to have a Globally
Asynchronous Locally Synchronous (GALS) architecture.  The semantics
of SITPNs that integrate multi-clock domains has not been formalized
yet. The multi-clock domain aspect also implies modifying the \hvhdl{}
semantics to take into account multiple clock signals in the
simulation loop.
