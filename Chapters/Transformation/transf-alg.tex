...
\item \texttt{cassoc}$(map,id,x)$ where $map$ is either a generic map,
  ...
  is an expression, a name (i.e. a simple or indexed identifier) or
...
\texttt{transition} design) in the absence of input places. If the set
of input places of $t$ is not empty, then the \texttt{ian} constant is
associated with the size of the set in the generic map $g_t$.
