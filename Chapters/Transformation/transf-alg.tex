...
model-to-text transformation, we want to point out the necessity of
automatizing the transformation. Judging by the appearance of the
...
\hilecop{} methodology do not start directly by writing down the
\vhdl{} code. The reasons are many. First, handling the
interconnections between PCIs and TCIs is simple enough when the
number of places and transitions of the input SITPN is few, however,
it becomes a lot more tedious with the increase of the size of
models. To give an example, the \textsf{Neurrinov}
company\footnote{\url{https://neurinnov.com/}}, which applies the
\hilecop{} methodology to the design of critical digital circuits, has
developed a digital circuit model for the control of the
...
PNs appears to be more fit for the task of digital architecture design
in comparison to plain source code, as it facilitates the discussions
...
Chapter~\ref{chap:hilecop}, there exists a \textsf{Java}
implementation of the \hilecop{} methodology. This implementation
performs the generation of \vhdl{} code from a SITPN model. However,
the algorithm of the transformation has never been documented, nor a
formal specification given. The following algorithm is one of the
...
\texttt{mm} constant to the maximal marking value returned by the $b$
function for place $p$. The input port map $i_p$ and the output port
map $o_p$ are initialized with empty sets.
