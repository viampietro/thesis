...
\begin{tikzpicture}
  ...
  {
    \includegraphics[keepaspectratio, width=0.1\linewidth]{abstract-model.eps}%
  };
  ...
  node{\it\bfseries \fontsize{5}{8}\selectfont correction}
  (abstract);
  ...
  {
    \includegraphics[keepaspectratio, width=0.1\linewidth]{flattened-pn.eps}%
  };
  ...
  sep=1mm] {
    ...
    \begin{tabular}{c}
      ...
      \fontsize{7}{8}\selectfont Source \\
      \fontsize{7}{8}\selectfont Code\\
    \end{tabular}
  };
  ...
  \node (labelvhdl) at ($(vhdl)-(0,1)$) {\fontsize{7}{8}\selectfont \textcircled{3}};
  ...
  \node (labelfpga) at ($(fpga)-(0,1)$) {
    \renewcommand{\arraystretch}{0.5}
    \begin{tabular}{c}
      \fontsize{7}{8}\selectfont FPGA\\
      \fontsize{7}{8}\selectfont implementation\\
      \fontsize{7}{8}\selectfont \textcircled{4}
    \end{tabular}
  };
  
  \draw[red, thick, dotted] ($(impl.north west)+(-0.5,0.1)$) rectangle
  ($(labelvhdl.south east)+(1,0)$);
