...
\begin{tikzpicture}
  ...
  \node[draw, rectangle, inner sep=2mm] (vhdl) at ($(pn.east)+(3,0)$) {
    \begin{tikzpicture}[remember picture]
      ...
      \draw[red] plot [smooth,tension=.5] coordinates { (compT0Out1.south)
        ($(compT0.south west)-(0,.2)$) ($(compP0.north west)+(0,.2)$) (clkP0.north)};

      \draw[red] plot [smooth,tension=.5] coordinates { (compP1Out0.south)
        ($(compP1.south west)-(.2,.2)$) ($(compT0.north west)+(-.2,.2)$) (clkT0.north)};

      \draw[red] plot [smooth,tension=.5] coordinates { (compP1Out2.south)
