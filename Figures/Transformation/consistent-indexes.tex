...
\begin{tikzpicture}
  ...
  \node (vhdl) at ($(pn.east)+(5,0)$)
  {
    \begin{circuitikz}
      ...
      no topmark, external pins width=0, anchor=west] (idp) at ($(idta.east)+(2.5,0)$) {\Large$id_p$};
      ...
      % PORT PINS
      \draw ($(idp.bpin 6)$) -- ++(-.5,0) node[anchor=east,inner sep=0pt]{\ssmall $post(t_b,p)$};
      \draw ($(idp.bpin 7)$) -- ++(-.5,0) node[anchor=east,inner sep=0pt]{\ssmall $post(t_a,p)$};
      \draw ($(idp.bpin 8)$) -- ++(-.5,0) node[anchor=east,inner sep=0pt]{\ssmall $post(t_c,p)$};
